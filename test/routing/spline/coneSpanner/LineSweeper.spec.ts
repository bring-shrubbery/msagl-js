...
function CheckVGOnPoint(
  p: Point,
  vg: VisibilityGraph,
  ps: Array<Point>,
  dir: Point,
) {
  const inCone = ps.filter(
    (q) => p.equal(q) == false && InCone(p, q, dir, Math.PI / 6),
  )
  const v = vg.FindVertex(p)
  expect(
    (inCone.length == 0 &&
      (v == null ||
        (v.OutEdges.count == 0 && Array.from(v.InEdges).length > 0))) ||
      v.OutEdges.count == 1,
  ).toBe(true)
}
