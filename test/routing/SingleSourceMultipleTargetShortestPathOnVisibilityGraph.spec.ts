...
test('ssmtsp', () => {
  ...
  const topVertex = vg.AddVertexP(
    upperVVs[upperVVs.length - 1].point.add(new Point(0, 1)),
  )

  VisibilityGraph.AddEdge(
    new VisibilityEdge(topVertex, upperVVs[upperVVs.length - 2]),
  )
  ...
  VisibilityGraph.AddEdge(
    new VisibilityEdge(middleVertex, upperVVs[upperVVs.length - 2]),
  )

  const sp = new SingleSourceMultipleTargetsShortestPathOnVisibilityGraph(
    origin,
    [upperVVs[upperVVs.length - 1], topVertex],
    vg,
  )
