...
export function vertex(p: Parallelogram, vertexPar: VertexId): Point {
	switch (vertexPar) {
		case VertexId.Corner:
			return p.corner;
		case VertexId.VertexA:
			return p.aPlusCorner;
		case VertexId.otherCorner:
			return p.otherCorner;
		case VertexId.VertexB:
			return p.bPlusCorner;
		default:
			return undefined;
	}
}

function PumpMinMax(minx: number, maxx: number, miny: number, maxy: number, p: Point) {
	if (p.x < minx) {
		minx = p.x;
	} else if (p.x > maxx) {
		maxx = p.x;
...
export function intersect(parallelogram0: Parallelogram, parallelogram1: Parallelogram): boolean {
	...
	const ret = !(
		separByA(parallelogram0, parallelogram1) ||
		separByA(parallelogram1, parallelogram0) ||
		separByB(parallelogram0, parallelogram1) ||
		separByB(parallelogram1, parallelogram0)
	);
	...
	if (
		!parallelWithinEpsilon(parallelogram0.otherCorner.minus(parallelogram0.corner), parallelogram1.otherCorner.minus(parallelogram1.corner), 1.0e-5)
	)
		return true;
...
function separByB(p0: Parallelogram, p1: Parallelogram) {
	...
	const p1a = vertex(p1, 0).minus(p0.corner).dot(p0.bRot);
	const list = [VertexId.VertexA, VertexId.otherCorner, VertexId.VertexB];
	if (p1a > p0.abRot + eps) {
		for (const i of list) {
			if (vertex(p1, i).minus(p0.corner).dot(p0.bRot) <= p0.abRot + eps) return false;
		}

		return true;
	} else if (p1a < -eps) {
		for (const i of list) {
			if (vertex(p1, i).minus(p0.corner).dot(p0.bRot) >= -eps) return false;
		}
		return true;
	}
	return false;
