...
export class Curve implements ICurve {
  ...
  trim(start: number, end: number): ICurve {
    ...
    for (let i = e.segIndex + 1; i < e.segIndex; i++) c = c.addSegment(this.segs[i]);
  ...
  static getAllIntersectionsOfLineAndArc(lineSeg: LineSegment, ellipse: Ellipse): IntersectionInfo[] {
    ...
    if (Point.closeD(rad, absSegProj)) {
      ...
      Curve.tryToAddPointToLineCircleCrossing(lineSeg, ellipse, ret, closestPointOnLine.minus(d), segLength, lineDir);
    }
    return ret;
  }
    ...
    for (let i = 1; i < 2; i++) {
      ...
      for (let j = 1; j < 2; j++) {
        ...
        if (l0.chord == null && l1.chord == null)
          ...
          r = Curve.crossWithinIntervalsWithGuess(l0.chord, n1.seg, 0, 1, l1.low, l1.high, 0.5 * i, p1, sol);
          if (r) {
          }
        } else if (l0.chord == null) {
          r = Curve.crossWithinIntervalsWithGuess(n0.seg, l1.chord, l0.low, l0.high, 0, 1, p0, 0.5 * j, sol);
          if (r) {
            sol.bSol = l1.low + sol.bSol * (l1.high - l1.low);
          }
        } //if (l0.chord != null && l1.chord != null)
        else {
          r = Curve.crossWithinIntervalsWithGuess(l0.chord, l1.chord, 0, 1, 0, 1, 0.5 * i, 0.5 * j, sol);
          if (r) {
    ...
    for (let i = 1; i < 2; i++) {
      ...
      for (let j = 1; j < 2; j++) {
        ...
        if (l0.chord == null && l1.chord == null)
          ...
          r = Curve.crossWithinIntervalsWithGuess(l0.chord, n1.seg, 0, 1, l1.low, l1.high, 0.5 * i, p1, sol);
          if (r) sol.aSol = l0.low + sol.aSol * (l0.high - l0.low);
        } else if (l0.chord == null) {
          //&& l1.chord != null)
          r = Curve.crossWithinIntervalsWithGuess(n0.seg, l1.chord, l0.low, l0.high, 0, 1, p0, 0.5 * j, sol);
          if (r) sol.bSol = l1.low + sol.bSol * (l1.high - l1.low);
        } //if (l0.chord != null && l1.chord != null)
        else {
          r = Curve.crossWithinIntervalsWithGuess(l0.chord, l1.chord, 0, 1, 0, 1, 0.5 * i, 0.5 * j, sol);
          if (r) {
