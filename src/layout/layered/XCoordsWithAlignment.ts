...
export class XCoordsWithAlignment {
  ...
  static CalculateXCoordinates(layerArrays: LayerArrays, layeredGraph: ProperLayeredGraph, nOfOriginalVs: number, anchors: Anchor[], nodeSeparation: number) {
  ...
  HorizontalBalancing() {
    ...
    for (let i: number = 0; (i < 4); i++) {
      ...
      if ((delta != 0)) {
        for (let j: number = 0; (j < this.nOfVertices); j++) {
    ...
    for (let v: number = 0; (v < this.nOfVertices); v++) {
  ...
  AssignmentBounds(i: number, t: { a: number, b: number }) {
    if ((this.nOfVertices == 0)) {
      ...
      t.b = this.x[0];
      t.a = this.x[0];
      for (let j: number = 1; (j < this.nOfVertices); j++) {
  ...
  *  UpperEdgeMedians(target: number): IterableIterator<number> {
  ...
  MarkConflictingEdges() {
    ...
    let lowest: number = i;
    let upperBound: number = this.UpperOf(0, (this.h - 1));
    let nextBelowUpperBound: number = this.NextLower(upperBound);
    ...
    for (
      ; this.IsBelow(i, upperBound); i = this.NextUpper(i)) {
  ...
  ConflictsWithAtLeastOneInnerEdgeForALayer(layerIndex: number) {
    if (((layerIndex >= 0)
      && (layerIndex < this.la.Layers.length))) {
      let lowerLayer: number[] = this.la.Layers[layerIndex];
      ...
      let lastTargetPos: number = this.RightMost(0, (lowerLayer.length - 1));

      for (
        ; (this.IsNotRightFrom(targetPos, lastTargetPos)
          && (innerEdge == null)); targetPos = this.NextRight(targetPos)) {
      ...
      if ((innerEdge != null)) {
        ...
        while (this.IsNotRightFrom(targetPos, lastTargetPos)) {
          ...
          let newInnerEdge: LayerEdge = this.AlignmentToTheRightOfInner(lowerLayer, targetPos, positionOfInnerEdgeSource);
          targetPos = this.NextRight(targetPos);
          if ((newInnerEdge != null)) {
            let newInnerSourcePos: number = this.Pos(this.Source(newInnerEdge));
        ...
        for (let k: number = this.NextRight(this.Pos(this.Target(innerEdge))); this.IsNotRightFrom(k, lastTargetPos); k = this.NextRight(k)) {
  ...
  MarkEdgesBetweenInnerAndNewInnerEdges(lowerLayer: number[], innerEdge: LayerEdge, newInnerEdge: LayerEdge, innerEdgeSourcePos: number, newInnerEdgeSourcePos: number) {
    let u: number = this.NextRight(this.Pos(this.Target(innerEdge)));
    for (
      ; this.IsLeftFrom(u, this.Pos(this.Target(newInnerEdge))); u = this.NextRight(u)) {
      for (const ie of this.InEdges(lowerLayer[u])) {
        let ieSourcePos: number = this.Pos(this.Source(ie));
  ...
  private AlignmentToTheRightOfInner(lowLayer: number[], i: number, posInnerSource: number): LayerEdge {
    let numOfInEdges: number = this.NumberOfInEdges(lowLayer[i]);
    if ((numOfInEdges == 1)) {
  ...
  private NumberOfInEdges(v: number): number {
    return this.BT ? this.graph.InEdgesCount(v) :
      this.graph.OutEdgesCount(v);
  }
  ...
  InnerEdgeByTarget(v: number): LayerEdge {
    if (this.IsVirtual(v)) {
      let ie: LayerEdge = this.InEdgeOfVirtualNode(v);
  ...
  AssignCoordinatesByLongestPath() {
    ...
    let blockGraph: BasicGraphOnEdges<PolyIntEdge> = new BasicGraphOnEdges<PolyIntEdge>().mkGraphOnEdgesN(edges, this.nOfVertices);
    // sort the graph in the topological order
    let topoSort: number[] = TopologicalSort.getOrderOnGraph(blockGraph);
    // start placing the blocks according to the order
    for (const v of topoSort) {
      if (v == this.root[v]) {
        let vx: number = 0;
        let vIsLeftMost: boolean = true;
        let w: number = v;
    ...
    for (const v of topoSort) {
      if ((v == this.root[v])) {
        if ((blockGraph.inEdges[v].length == 0)) {
          ...
          let xLeftMost: number = this.RightMost((XCoordsWithAlignment.infinity * -1), XCoordsWithAlignment.infinity);
          let xl: number = xLeftMost;
          for (
            ; (w != v);
          ) {
  ...
  TryToGetRightNeighbor(u: number, t: { neighbor: number }): boolean {
    let neighborPos: number = this.NextRight(this.Pos(u));
    let layer: number[] = this.la.Layers[this.la.Y[u]];
    if (((neighborPos >= 0)
      && (neighborPos < layer.length))) {
  ...
  TryToGetLeftNeighbor(u: number, t: { neighbor: number }): boolean {
    let neighborPos: number = this.NextLeft(this.Pos(u));
    let layer: number[] = this.la.Layers[this.la.Y[u]];
    if (((neighborPos >= 0)
      && (neighborPos < layer.length))) {
      t.neighbor = layer[neighborPos];
  ...
  CreateBlocks() {
    ...
    let lowBound: number = this.LowerOf(0, (this.h - 1));
    ...
    for (let i: number = this.NextLower(this.UpperOf(0, (this.h - 1))); !this.IsBelow(i, lowBound); i = this.NextLower(i)) {
      let layer: number[] = this.la.Layers[i];
  ...
  DeltaBetweenVertices(u: number, v: number): number {
    ...
    let anchorSepar: number = (this.anchors[u].rightAnchor + this.anchors[v].leftAnchor);
