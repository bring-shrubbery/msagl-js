...
export class XCoordsWithAlignment {
  ...
  static CalculateXCoordinates(
    ...
    const x = new XCoordsWithAlignment(
      layerArrays,
      layeredGraph,
      nOfOriginalVs,
      anchors,
  ...
  *UpperEdgeMedians(target: number): IterableIterator<number> {
    const medians = this.BT
      ? this.upperMedians[target]
      : this.lowMedians[target]
  ...
  ConflictsWithAtLeastOneInnerEdgeForALayer(layerIndex: number) {
    if (layerIndex >= 0 && layerIndex < this.la.Layers.length) {
      ...
      for (
        ;
        this.IsNotRightFrom(targetPos, lastTargetPos) && innerEdge == null;
      ...
      if (innerEdge != null) {
        ...
        for (
          let j: number = this.LeftMost(0, lowerLayer.length - 1);
          ...
          for (const ie of this.InEdges(lowerLayer[j])) {
            if (
              this.IsLeftFrom(
                positionOfInnerEdgeSource,
                this.Pos(this.Source(ie)),
              )
            ) {
              this.MarkEdge(ie)
            }
        ...
        while (this.IsNotRightFrom(targetPos, lastTargetPos)) {
          ...
          const newInnerEdge: LayerEdge = this.AlignmentToTheRightOfInner(
            lowerLayer,
            targetPos,
            positionOfInnerEdgeSource,
          )
          ...
          if (newInnerEdge != null) {
            const newInnerSourcePos: number = this.Pos(
              this.Source(newInnerEdge),
            )
            this.MarkEdgesBetweenInnerAndNewInnerEdges(
              lowerLayer,
              innerEdge,
              newInnerEdge,
              innerSourcePos,
        ...
        for (
          ...
          for (const ie of this.InEdges(lowerLayer[k])) {
            if (
              this.IsLeftFrom(
                this.Pos(this.Source(ie)),
                this.Pos(this.Source(innerEdge)),
              )
            ) {
              this.MarkEdge(ie)
            }
  ...
  InEdgeOfVirtualNode(v: number): LayerEdge {
    return this.BT
      ? this.graph.InEdgeOfVirtualNode(v)
      : this.graph.OutEdgeOfVirtualNode(v)
  }
  ...
  MarkEdgesBetweenInnerAndNewInnerEdges(
    ...
    for (
      ;
      this.IsLeftFrom(u, this.Pos(this.Target(newInnerEdge)));
      u = this.NextRight(u)
    ) {
  ...
  private AlignmentToTheRightOfInner(
    ...
    if (numOfInEdges == 1) {
      ...
      if (
        this.IsInnerEdge(ie) &&
        this.IsLeftFrom(posInnerSource, this.Pos(ie.Source))
      ) {
        return ie
      }
    ...
    for (const v of topoSort) {
      if (v == this.root[v]) {
        if (blockGraph.inEdges[v].length == 0) {
          ...
          let xLeftMost: number = this.RightMost(
            XCoordsWithAlignment.infinity * -1,
  ...
  CreateBlocks() {
    ...
    for (
      let i: number = this.NextLower(this.UpperOf(0, this.h - 1));
      !this.IsBelow(i, lowBound);
      i = this.NextLower(i)
    ) {
      const layer: number[] = this.la.Layers[i]
      let r: number = this.LeftMost(
        -1,
        this.la.Layers[this.NextUpper(i)].length,
      )
      ...
      for (
        let k: number = this.LeftMost(0, layer.length - 1);
        this.IsNotRightFrom(k, rightBound);
        k = this.NextRight(k)
      ) {
