...
export class XLayoutGraph extends BasicGraphOnEdges<PolyIntEdge> {
  ...
  weightMultiplierOfOriginalOriginal: number = 1;

  // weight multiplier for edges with Defaults or n end and start
  weightMultOfOneVirtual: number = 3;

  // weight multiplier for edges with only one virtual node
  weightMultiplierOfTwoVirtual: number = 8;

  // weight multiplier for edges with two virtual nodes
  /* internal */ constructor(graph: BasicGraphOnEdges<PolyIntEdge>, layeredGraph: ProperLayeredGraph, layerArrays: LayerArrays, edges: PolyIntEdge[], nov: number) {
    super()
  ...
  EdgeWeightMultiplier(edge: PolyIntEdge): number {
    let s: number = edge.source;
    let t: number = edge.target;
    if (((s < this.layeredGraph.NodeCount)
      && ((this.layerArrays.Y[s] == this.layerArrays.Y[t])
        && (this.layerArrays.X[s]
          == (this.layerArrays.X[t] + 1))))) {
      return 0;
    }
    ...
    if (s0 >= this.virtualVerticesStart
      && s0 <= this.virtualVerticesEnd) {
      k++;
    }

    if (t0 >= this.virtualVerticesStart
      && t0 <= this.virtualVerticesEnd) {
      k++;
    }

    let ret = k == 0 ? this.weightMultiplierOfOriginalOriginal : (k == 1 ? this.weightMultOfOneVirtual : this.weightMultiplierOfTwoVirtual);
    return ret;
  }
