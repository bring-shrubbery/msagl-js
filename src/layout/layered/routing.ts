...
export class Routing extends Algorithm {
    ...
    for (const [k, v] of this.Database.Multiedges.keyValues()) {
      ...
      if ((ip.x == ip.y)) {
        ...
        for (const intEdge of v.) {
          this.ProgressStep();
          const dx: number = (this.settings.NodeSeparation
            + (this.settings.MinNodeWidth + offset));
          ...
          if (intEdge.edge.label != null) {
            ...
            const center = intEdge.edge.label.center =
              new Point(c.value((c.parStart + c.parEnd) / 2).x + intEdge.LabelWidth / 2,
                anchor.y);
          ...
          Arrowhead.trimSplineAndCalculateArrowheadsII(intEdge.Edge.EdgeGeometry, intEdge.Edge.source.BoundaryCurve, intEdge.Edge.target.BoundaryCurve, c, false);
        }
  ...
  CreateSplineForNonSelfEdge(es: PolyIntEdge, optimizeShortEdges: boolean) {
    ...
    if ((es.LayerEdges != null)) {
      ...
      if (!es.IsVirtualEdge) {
        ...
        Arrowhead.trimSplineAndCalculateArrowheadsII(es.edge.edgeGeometry, es.edge.source.boundaryCurve, es.edge.target.boundaryCurve, es.curve, true);
      }
    }
  }

  DrawSplineBySmothingThePolyline(edgePath: PolyIntEdge, optimizeShortEdges: boolean) {
    const smoothedPolyline = new SmoothedPolylineCalculator(edgePath, this.Database.Anchors, this.OriginalGraph, this.settings, this.LayerArrays, this.ProperLayeredGraph, this.Database);
  ...
  static UpdateLabel(e: Edge, anchor: Anchor) {
    ...
    if ((Curve.GetAllIntersections(e.Curve, Curve.PolyFromBox(e.LabelBBox), false).length == 0)) {
      ...
      if (Routing.FindClosestPoints(/* out */curveClosestPoint, /* out */labelSideClosest, segmentInFrontOfLabel, labelSide)) {
        // shift the label if needed
        Routing.ShiftLabel(e, /* ref */curveClosestPoint, /* ref */labelSideClosest);
      }
      else {
        ...
        if (((segmentInFrontOfLabel[u] - labelSide.Start).Length < (segmentInFrontOfLabel[v] - labelSide.End).Length)) {
  ...
  static ShiftLabel(e: Edge, /* ref */curveClosestPoint: Point, /* ref */labelSideClosest: Point) {
  ...
  static FindClosestPoints(/* out */curveClosestPoint: Point, /* out */labelSideClosest: Point, segmentInFrontOfLabel: ICurve, labelSide: LineSegment): boolean {
    const v: number;
    const u: number;
    return Curve.MinDistWithinIntervals(segmentInFrontOfLabel, labelSide, segmentInFrontOfLabel.ParStart, segmentInFrontOfLabel.ParEnd, labelSide.ParStart, labelSide.ParEnd, ((segmentInFrontOfLabel.ParStart + segmentInFrontOfLabel.ParEnd)
      / 2), ((labelSide.ParStart + labelSide.ParEnd)
        / 2), /* out */u, /* out */v, /* out */curveClosestPoint, /* out */labelSideClosest);
  }
