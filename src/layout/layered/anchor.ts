...
export class Anchor {
  // ToString
  toString() {
    return "la:ra " + this.la + " " + this.ra + " ta:ba " + this.ta + " " + this.ba + " x:y " + this.x + " " + this.y
  }
  ...
  get left() { return this.x - this.la; }


  // right boundary of the node
  get right() { return this.x + this.ra; }

  // top boundary of the node
  get top() { return this.y + this.ta; }
  set top(value) { this.y += value - this.ta; }


  // bottom of the node
  get bottom() { return this.y - this.ba; }
  set bottom(value) { this.y += value - this.ba; }
  ...
  // constructor
  mkAnchor(leftAnchor: number, rightAnchor: number,
    topAnchor: number, bottomAnchor: number, node: GeomNode, labelCornersPreserveCoefficient: number) {
  ...
  get width() { return this.la + this.ra; }

  get height() { return this.ta + this.ba; }
  ...
  static getPaddedCorner(first: PolylinePoint, second: PolylinePoint, third: PolylinePoint, padding: number): {
    ...
    const uvPerp = v.sub(u).rotate(Math.PI / 2).normalize();
  ...
  static curveIsConvex(poly: Polyline) {
    const orientation = Point.getTriangleOrientation(poly.endPoint.point, poly.startPoint.point, poly.startPoint.next.point);
  ...
  polygonOnRightLabel() {
    ...
    const poly = Polyline.mkFromPoints([
      new Point(t, this.top)
      , new Point(this.right, this.Y), new Point(t, this.bottom)
      , this.leftBottom
      , this.leftTop])
    poly.closed = true
    return poly
