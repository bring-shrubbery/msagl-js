...
export class MetroMapOrdering {
  ...
  static UpdateLayerArrays1(properLayeredGraph: ProperLayeredGraph, layerArrays: LayerArrays) {
    const nodePositions: Map<number, Point> = MetroMapOrdering.BuildInitialNodePositions(properLayeredGraph, layerArrays);
  ...
  static BuildInitialNodePositions(properLayeredGraph: ProperLayeredGraph, layerArrays: LayerArrays): Map<number, Point> {
    let result = new Map<number, Point>();
    for (let i: number = 0; (i < layerArrays.Layers.length); i++) {
      let curr: number = 0;
      let prev: number = 0;
  ...
  CreateInitialOrdering(): PointMap<Array<number>> {
    let initialOrdering = new PointMap<Array<number>>();
    for (let i: number = 0; (i < this.layerArrays.Layers.length); i++) {
      for (let j: number = 0; (j < this.layerArrays.Layers[i].length); j++) {
        let node: number = this.layerArrays.Layers[i][j];
        let p = this.nodePositions.get(node)
  ...
  BuildOrdering(initialOrdering: PointMap<Array<number>>): PointMap<Array<number>> {
    ...
    let result = new PointMap<Array<number>>();
    let reverseOrder = new Map<number, number>();
    for (let i: number = 0; (i < this.layerArrays.Layers.length); i++) {
      for (let j: number = 0; (j < this.layerArrays.Layers[i].length); j++) {
        let node: number = this.layerArrays.Layers[i][j];
        // already processed
        ...
        result[this.nodePositions[node]] = this.BuildNodeOrdering(initialOrdering[this.nodePositions[node]], reverseOrder);
      }
  ...
  BuildNodeOrdering(nodeOrdering: Array<number>, inverseToOrder: Map<number, number>): Array<number> {
    let result: Array<number> = nodeOrdering;
  ...
  Comparison(inverseToOrder: Map<number, number>) {
    return ((node1: number, node2: number) => {
      ...
      Assert.assert((this.properLayeredGraph.IsVirtualNode(node1) && this.properLayeredGraph.IsVirtualNode(node2)));
      let succ1: number = from(this.properLayeredGraph.Succ(node1)).first();
      let succ2: number = from(this.properLayeredGraph.Succ(node2)).first();
      ...
      let succPoint1: Point = this.nodePositions[succ1];
      let succPoint2: Point = this.nodePositions[succ2];
      let predPoint1: Point = this.nodePositions[pred1];
      let predPoint2: Point = this.nodePositions[pred2];
      ...
      if (this.properLayeredGraph.IsVirtualNode(succ1)) {
        ...
        let o1: number = inverseToOrder[succ1];
        let o2: number = inverseToOrder[succ2];
        Assert.assert(((o1 != -1)
          && (o2 != -1)));
  ...
  RestoreLayerArrays(ordering: PointMap<Array<number>>) {
    for (let i: number = 0; (i < this.layerArrays.Layers.length); i++) {
      ...
      while ((tec < this.layerArrays.Layers[i].length)) {
        while (((tec < this.layerArrays.Layers[i].length)
          && (this.nodePositions[this.layerArrays.Layers[i][pred]] == this.nodePositions[this.layerArrays.Layers[i][tec]]))) {
          tec++;
        }

        for (let j: number = pred; (j < tec); j++) {
          this.layerArrays.Layers[i][j] = ordering[this.nodePositions[this.layerArrays.Layers[i][j]]][(j - pred)];
        }
