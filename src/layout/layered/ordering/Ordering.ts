...
namespace Microsoft.Msagl.Layout.Layered {
    ...
    // Works on the layered graph. Also see GraphLayout.pdfhttps://www.researchgate.net/profile/Lev_Nachmanson/publication/30509007_Drawing_graphs_with_GLEE/links/54b6b2930cf2e68eb27edf71/Drawing-graphs-with-GLEE.pdf
     partial class Ordering : AlgorithmBase{
        #region Fields
        ...
        number startOfVirtNodes;

        bool tryReverse = true;

        number NoGainStepsBound {
            get {
        ...
        Ordering(ProperLayeredGraph graphPar, bool tryReverse, LayerArrays layerArraysParam, number startOfVirtualNodes, bool balanceVirtualAndOrigNodes, bool hasCrossWeights, SugiyamaLayoutSettings settings) {
      ...
      for (IntEdge e in graph.Edges)
        ...
        void Calculate() {
            ...
            for (number i = 0; i < MaxOfIterations && countOfNoGainSteps < NoGainStepsBound && !measure.IsPerfect(); i++) {
                this.ProgressStep();
         ...
         static void CloneLayers(number[][] layers, ref number[][] layerArraysCopy) {
            if (layerArraysCopy == null) {
                layerArraysCopy = (number[][]) layers.Clone();
                for (number i = 0; i < layers.Length; i++)
                    layerArraysCopy[i] = (number[]) layers[i].Clone();
            } else
                for (number i = 0; i < layers.Length; i++)
        ...
        void LayerByLayerSweep(bool up) {
            if (up) {
                for (number i = 1; i < nOfLayers; i++)
                    SweepLayer(i, true);
            } else
                for (number i = nOfLayers - 2; i >= 0; i--)
                    SweepLayer(i, false);
        }
        ...
        // of the strip</param>
        void SweepLayer(number layer, bool upperLayer) {
            this.ProgressStep();

            number[] l = layers[layer];
            var medianValues = new float[l.Length];

            for (number i = 0; i < medianValues.Length; i++)
                medianValues[i] = WMedian(l[i], upperLayer);

            Sort(layer, medianValues);

            //update X
            number[] vertices = layerArrays.Layers[layer];
        ...
        void Sort(number layerToSort, float[] medianValues) {
            var s = new SortedDictionary<float, object>();
            number[] vertices = layers[layerToSort];
            number i = 0;

            for (float m in medianValues) {
                number v = vertices[i++];
                if (m == -1.0)
                    continue;
                ...
                if (!s.ContainsKey(m))
                    ...
                    object o = s[m];
                    var al = o as List<number>;
                    if (al != null) {
                        if (HeadOfTheCoin())
                            al.Add(v);
                        else {
//stick it in the middle 
                            number j = random.Next(al.Count);
                            number k = al[j];
                            al[j] = v;
                            al.Add(k);
                        }
...
//find the first empty spot
        ...
        float WMedian(number node, bool theMedianGoingDown) {
        ...
        void Init() {
            ...
            while (q.Count > 0) {
                ...
                for (number v in properLayeredGraph.Succ(u))
                    if (!visited[v]) {
                        visited[v] = true;
                        q.Push(v);
                    }
