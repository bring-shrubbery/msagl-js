...
export class ProperLayeredGraph {
  ...
  BaseGraph: BasicGraph<Node, PolyIntEdge>
  ...
  constructor(intGraph: BasicGraph<Node, PolyIntEdge>) {
    this.Initialize(intGraph);
  }

  Initialize(intGraph: BasicGraph<Node, PolyIntEdge>) {
    this.BaseGraph = intGraph;

    if (this.BaseGraph.edges.length > 0) {
      const edgesGoingDown = from(this.BaseGraph.edges).where(edge => edge.layerEdges != null)
    ...
    if (ProperLayeredGraph.ExistVirtualNodes(from(this.BaseGraph.edges))) {
      this.firstVirtualNode = from(this.BaseGraph.edges.filter(e => e.layerEdges != null && e.layerEdges.length > 1)).selectMany(e => from(e.layerEdges).where(le => le.source != e.source)).select(le => le.source).min()
  ...
  static ExistVirtualNodes(iCollection: IEnumerable<PolyIntEdge>): boolean {
    return iCollection.any(edge => edge.layerEdges != null && edge.layerEdges.length > 1)
  }
  ...
  *OutEdges(node: number): IterableIterator<LayerEdge> {
  ...
  public bool InDegreeIsMoreThanOne(number node) {
    if (node < this.BaseGraph.nodeCount)//original node
