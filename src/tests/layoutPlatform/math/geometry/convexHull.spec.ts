...
test('repeated points', () => {
  ...
  for (let i = 0; i < 20; i++) {
    for (const p of expected) points.push(p)
  }

  const actual = Array.from(ConvexHull.CalculateConvexHull(points))
...
function VerifyAndDisplayConvexHull(points: Point[], filename: string) {
  ...
  SvgDebugWriter.dumpDebugCurves('/tmp/' + filename, [
    DebugCurve.mkDebugCurveTWCI(100, 0.01, 'magenta', hull),
