...
export class Block {
  ...
  ComputeDfDv(initialVarToEval: Variable) {
    ...
    const firstNode = this.GetDfDvNode(
  ...
  ProcessDfDvLeafNode(node: DfDvNode) {
    ...
    if (node.IsLeftToRight) {
      ...
      node.Parent.ConstraintToEval.Lagrangian =
        node.Parent.ConstraintToEval.Lagrangian +
        node.ConstraintToEval.Lagrangian
    } else {
      ...
      node.ConstraintToEval.Lagrangian =
        (node.ConstraintToEval.Lagrangian + dfdv) * -1
      node.Parent.ConstraintToEval.Lagrangian =
        node.Parent.ConstraintToEval.Lagrangian -
        node.ConstraintToEval.Lagrangian
    }
  ...
  Expand(violatedConstraint: Constraint) {
      ...
      for (const pathItem of this.constraintPath) {
        if (
          ...
          (minLagrangianConstraint == null ||
            pathItem.Constraint.Lagrangian < minLagrangianConstraint.Lagrangian)
        ) {
    ...
    this.GetConnectedVariables(
      lstConnectedVars,
      violatedConstraint.Right,
      violatedConstraint.Left,
    )
    ...
    for (let ii = 0; ii < cConnectedVars; ii++) {
      lstConnectedVars[ii].OffsetInBlock =
  ...
  Split(isQpsc: boolean): Block {
    ...
    for (let ii = 0; ii < numVars; ii++) {
      for (const constraint of this.Variables[ii].LeftConstraints) {
        if (
          constraint.IsActive &&
          !constraint.IsEquality &&
          constraint.Lagrangian < minLagrangian
        ) {
  ...
  RecurseGetConnectedVariables(
    lstVars: Array<Variable>,
    initialVarToEval: Variable,
    initialVarDoneEval: Variable,
  ) {
