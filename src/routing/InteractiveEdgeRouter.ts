...
export class InteractiveEdgeRouter extends Algorithm {
    ...
    if (this.sourcePort != null) {
      ...
      if (this.sourcePort instanceof FloatingPort) {
        ...
        this.StartPointOfEdgeRouting =
          this.TakeBoundaryPortOutsideOfItsLoosePolyline(
            bp.Curve,
            bp.Parameter,
  ...
  ExtendVisibilityGraphToLocation(location: Point) {
    ...
    if (addedPolygons == null || addedPolygons.length == 0) {
      ...
      const visibilityGraphGenerator =
        new InteractiveTangentVisibilityGraphCalculator(
          addedPolygons,
          this.activePolygons,
  ...
  TakeBoundaryPortOutsideOfItsLoosePolyline(
    ...
    if (
      Point.getTriangleOrientation(
        InteractiveEdgeRouter.PointInsideOfConvexCurve(nodeBoundary),
        location,
        location.add(tangent),
  ...
  *GetActivePolylines(): IterableIterator<Polyline> {
  ...
  GetAddedPolygonesAndMaybeExtendActiveRectangle(): Array<Polygon> {
    ...
    for (; added; ) {
      added = false
      for (const loosePoly of this.ObstacleCalculator.RootOfLooseHierarchy.GetNodeItemsIntersectingRectangle(
  ...
  RelaxPolyline() {
    let relaxedPolylinePoint =
      InteractiveEdgeRouter.CreateRelaxedPolylinePoints(this._polyline)
    for (
  ...
  RelaxPolylinePoint(relaxedPoint: RelaxedPolylinePoint) {
    ...
    for (
      ...
      d > GeomConstants.distanceEpsilon &&
      !this.RelaxWithGivenOffset(d, relaxedPoint);

    ) {}
  }

  RelaxWithGivenOffset(
    offset: number,
    relaxedPoint: RelaxedPolylinePoint,
  ): boolean {
  ...
  static PullCloserRelaxedPoint(relaxedPolylinePoint: RelaxedPolylinePoint) {
    relaxedPolylinePoint.PolylinePoint.point =
      relaxedPolylinePoint.OriginalPosition.mul(0.2).add(
        relaxedPolylinePoint.PolylinePoint.point.mul(0.8),
      )
  }

  StickingSegmentDoesNotIntersectTightObstacles(
    relaxedPoint: RelaxedPolylinePoint,
  ): boolean {
    return (
      !this.PolylineSegmentIntersectsTightHierarchy(
        relaxedPoint.PolylinePoint.point,
  ...
  PolylineSegmentIntersectsTightHierarchy(a: Point, b: Point): boolean {
    return this.PolylineIntersectsPolyRectangleNodeOfTightHierarchyPPR(
      a,
      b,
      this.ObstacleCalculator.RootOfTightHierarchy,
    )
  ...
  PolylineIntersectsPolyRectangleNodeOfTightHierarchyPPR(
    a: Point,
    b: Point,
    rect: RectangleNode<Polyline, Point>,
  ): boolean {
    return this.PolylineIntersectsPolyRectangleNodeOfTightHierarchy(
      LineSegment.mkPP(a, b),
      rect,
    )
  }

  PolylineIntersectsPolyRectangleNodeOfTightHierarchy(
    ls: LineSegment,
    rect: RectangleNode<Polyline, Point>,
  ): boolean {
    ...
    if (rect.UserData != null) {
      for (const ii of Curve.getAllIntersections(ls, rect.UserData, false)) {
        if (
          ii.seg1 != this.SourceTightPolyline &&
          ii.seg1 != this.TargetTightPolyline
        ) {
          return true
        }

        if (
          (ii.seg1 == this.SourceTightPolyline && this.SourcePort) instanceof
          CurvePort
        ) {
          return true
        }
        ...
        if (
          (ii.seg1 == this.TargetTightPolyline && this.TargetPort) instanceof
          CurvePort
        ) {
          return true
        }
  ...
  static IntersectionsOfLineAndRectangleNodeOverPolylineLR(
    ...
    InteractiveEdgeRouter.IntersectionsOfLineAndRectangleNodeOverPolyline(
      ls,
      rectNode,
      ret,
    )
  ...
  static IntersectionsOfLineAndRectangleNodeOverPolyline(
    ...
    if (rectNode.UserData != null) {
      addRange(
        listOfIntersections,
        Curve.getAllIntersections(ls, rectNode.UserData, true),
      )
      return
    }
  ...
  LineCanBeAcceptedForRouting(ls: LineSegment): boolean {
    ...
    const xx: Array<IntersectionInfo> =
      InteractiveEdgeRouter.IntersectionsOfLineAndRectangleNodeOverPolylineLR(
        ls,
        this.ObstacleCalculator.RootOfTightHierarchy,
      )
    for (const ii of xx) {
  ...
  InsideOfTheAllowedConeOfBoundaryPort(
    pointToTest: Point,
    port: CurvePort,
  ): boolean {
    const boundaryCurve: ICurve = port.Curve
    const curveIsClockwise: boolean =
      InteractiveObstacleCalculator.CurveIsClockwise(
        boundaryCurve,
        InteractiveEdgeRouter.PointInsideOfConvexCurve(boundaryCurve),
      )
    const portLocation: Point = port.Location
    const pointOnTheRightConeSide: Point =
      this.GetPointOnTheRightBoundaryPortConeSide(
        portLocation,
        boundaryCurve,
        curveIsClockwise,
        port.Parameter,
      )
    const pointOnTheLeftConeSide: Point =
      this.GetPointOnTheLeftBoundaryPortConeSide(
        portLocation,
        boundaryCurve,
        curveIsClockwise,
        port.Parameter,
    ...
    return (
      ...
      Point.getTriangleOrientation(
        portLocation,
        pointToTest,
        pointOnTheLeftConeSide,
      ) != TriangleOrientation.Clockwise
    )
  }

  GetPointOnTheRightBoundaryPortConeSide(
    portLocation: Point,
    boundaryCurve: ICurve,
    curveIsClockwise: boolean,
    portParam: number,
  ): Point {
    const tan: Point = curveIsClockwise
      ? boundaryCurve.rightDerivative(portParam)
      : boundaryCurve.leftDerivative(portParam).neg()

    return portLocation.add(tan.rotate(this.EnteringAngleBound))
  }

  GetPointOnTheLeftBoundaryPortConeSide(
    portLocation: Point,
    boundaryCurve: ICurve,
    curveIsClockwise: boolean,
    portParam: number,
  ...
  static SetRelaxedPointLocation(
    offset: number,
    relaxedPoint: RelaxedPolylinePoint,
  ) {
    const leftTurn: boolean =
      Point.getTriangleOrientation(
        relaxedPoint.Next.OriginalPosition,
        relaxedPoint.OriginalPosition,
        relaxedPoint.Prev.OriginalPosition,
      ) == TriangleOrientation.Counterclockwise
    let v: Point = relaxedPoint.Next.OriginalPosition.sub(
      relaxedPoint.Prev.OriginalPosition,
    )
      .normalize()
      .mul(offset)
  ...
  SmoothOneCorner(a: CornerSite, c: CornerSite, b: CornerSite): CornerSite {
    ...
    for (
      ;
      this.ObstacleCalculator.ObstaclesIntersectICurve(seg) && k > kMin;
  ...
  TryToRemoveInflectionsAndCollinearSegments(
    ...
    while (progress) {
      progress = false
      for (
        t.s = underlyingPolyline.headSite;
        t.s != null && t.s.next != null;
        t.s = t.s.next
      ) {
  ...
  TryToRemoveInflectionEdge(t: {s: CornerSite}): boolean {
    if (
      !this.ObstacleCalculator.ObstaclesIntersectLine(
        t.s.prev.point,
        t.s.next.point,
      )
    ) {
  ...
  GetShortestPolyline(
    ...
    _targetVisVertex: VisibilityVertex,
  ): Polyline {
    this.CleanTheGraphForShortestPath()
    const pathCalc = new SingleSourceSingleTargetShortestPathOnVisibilityGraph(
      this.visibilityGraph,
      sourceVisVertex,
      _targetVisVertex,
    )
    ...
    Assert.assert(
      path[0] == sourceVisVertex && path[path.length - 1] == _targetVisVertex,
    )
  ...
  RouteEdgeToLocation(targetLocation: Point): EdgeGeometry {
    ...
    const edgeGeometry = new EdgeGeometry()
    ...
    if (this.LineCanBeAcceptedForRouting(ls)) {
      ...
      edgeGeometry.smoothedPolyline = SmoothedPolyline.mkFromPoints(
        this._polyline,
      )
    ...
    this._polyline = this.GetShortestPolyline(
      this.SourceVisibilityVertex,
      this.TargetVisibilityVertex,
    )
    this.RelaxPolyline()
    if (this.SourcePort instanceof CurvePort) {
  ...
  RouteEdgeToPort(
    ...
    if (!this.ObstacleCalculator.IsEmpty()) {
      ...
      const bp = <CurvePort>edgeTargetPort
      if (bp != null) {
    ...
    if (this.sourcePort != null && this.targetPort != null) {
      t.smoothedPolyline = this.SmoothedPolylineFromTwoPoints(
        this.sourcePort.Location,
  ...
  RouteEdgeToFloatingPortOfNode(
    ...
    if (this.sourcePort instanceof FloatingPort) {
      return this.RouteFromFloatingPortToFloatingPort(
        portLoosePolyline,
        smooth,
        t,
      )
    ...
    return this.RouteFromBoundaryPortToFloatingPort(
      portLoosePolyline,
      smooth,
      t,
    )
  ...
  RouteFromBoundaryPortToFloatingPort(
    ...
    this.ExtendVisibilityGraphToLocationOfTargetFloatingPort(
      targetPortLoosePolyline,
    )
    this._polyline = this.GetShortestPolyline(
      this.SourceVisibilityVertex,
      this.TargetVisibilityVertex,
  ...
  RouteFromFloatingPortToFloatingPort(
    ...
    const targetPortLocation: Point = this.TargetPort.Location
    const ls = LineSegment.mkPP(
      this.StartPointOfEdgeRouting,
      targetPortLocation,
    )
    if (
      this.AllowedShootingStraightLines &&
      this.LineAvoidsTightHierarchyLPP(
        ls,
  ...
  TryShortcutPolylineEnd() {
    ...
    if (
      this.LineAvoidsTightHierarchyPPPP(
        a.point,
        m,
        this._sourceTightPolyline,
        this.targetTightPolyline,
      )
    ) {
  ...
  ShortcutPolylineOneTime(): boolean {
    let ret = false
    for (
      let pp: PolylinePoint = this._polyline.startPoint;
      pp.next != null && pp.next.next != null;
      pp = pp.next
    ) {
  ...
  TryShortcutPolyPoint(pp: PolylinePoint): boolean {
    if (
      this.LineAvoidsTightHierarchyLPP(
        LineSegment.mkPP(pp.point, pp.next.next.point),
        this.SourceTightPolyline,
        this.targetTightPolyline,
      )
    ) {
  ...
  ExtendVisibilityGraphToLocationOfTargetFloatingPort(
    portLoosePolyline: Polyline,
  ) {
    if (this.VisibilityGraph == null) {
      this.VisibilityGraph = new VisibilityGraph()
    ...
    if (addedPolygons == null) {
      ...
      this.CalculateEdgeTargetVisibilityGraphForFloatingPort(
        targetLocation,
        portLoosePolyline,
      )
  ...
  CalculateEdgeTargetVisibilityGraphForFloatingPort(
    ...
    if (this.UseSpanner) {
      this.targetVisibilityVertex = this.AddTransientVisibilityEdgesForPort(
        targetLocation,
        targetLoosePoly,
      )
    } else {
  ...
  AddTransientVisibilityEdgesForPort(
    point: Point,
    loosePoly: Iterable<Point>,
  ): VisibilityVertex {
  ...
  *GetActivePolylinesWithException(
    targetLoosePoly: Polyline,
...
return from polygon in activePolygons where polygon.Polyline != targetLoosePoly select polygon.Polyline;
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            Point.closeDistEps(this.StartPointOfEdgeRouting, takenOutPoint)
          ) {
            this._polyline = new Polyline()
            this._polyline.addPoint(sourcePortLocation)
            ...
            this.LineAvoidsTightHierarchy(
              LineSegment.mkPP(this.StartPointOfEdgeRouting, takenOutPoint),
            )
          ) {
            ...
            const tmpSourceTight: Polyline =
              this.HideSourceTargetTightsIfNeeded(r)
            this.TryShortcutPolyline()
  ...
  RecoverSourceTargetTights(
    tmpSourceTight: Polyline,
  ...
  RouteFromFloatingPortToBoundaryPort(
    smooth: boolean,
    r: {smoothedPolyline: SmoothedPolyline},
  ): ICurve {
    const targetPortLocation: Point = this.targetPort.Location
    let ls: LineSegment
    if (
      ...
      if (this.LineCanBeAcceptedForRouting(ls)) {
        r.smoothedPolyline = this.SmoothedPolylineFromTwoPoints(
          ls.start,
          ls.end,
        )
        return ls
    ...
    const takenOutTargetPortLocation: Point =
      this.TakeBoundaryPortOutsideOfItsLoosePolyline(
        this.TargetPort.Curve,
        (<CurvePort>this.TargetPort).Parameter,
        this.TargetLoosePolyline,
    ...
    if (this.LineAvoidsTightHierarchyLP(ls, this._sourceTightPolyline)) {
      this._polyline = Polyline.mkFromPoints([
        ls.start,
        ls.end,
        targetPortLocation,
      ])
  ...
  LineAvoidsTightHierarchyLP(
    ls: LineSegment,
    polylineToExclude: Polyline,
  ): boolean {
    let lineIsGood = true
    for (const ii of InteractiveEdgeRouter.IntersectionsOfLineAndRectangleNodeOverPolylineLR(
      ls,
  ...
  LineAvoidsTightHierarchyLPP(
    ls: LineSegment,
    polylineToExclude0: Polyline,
    polylineToExclude1: Polyline,
  ): boolean {
    let lineIsGood = true
    for (const ii of InteractiveEdgeRouter.IntersectionsOfLineAndRectangleNodeOverPolylineLR(
      ls,
  ...
  LineAvoidsTightHierarchyPPPP(
    a: Point,
    b: Point,
    polylineToExclude0: Polyline,
    polylineToExclude1: Polyline,
  ): boolean {
    return this.LineAvoidsTightHierarchyLPP(
      LineSegment.mkPP(a, b),
      polylineToExclude0,
      polylineToExclude1,
    )
  ...
  GetHitLoosePolyline(point: Point): Polyline {
    if (
      this.ObstacleCalculator.IsEmpty() ||
      this.ObstacleCalculator.RootOfLooseHierarchy == null
    ) {
      return null
    }

    return InteractiveEdgeRouter.GetFirstHitPolyline(
      point,
      this.ObstacleCalculator.RootOfLooseHierarchy,
    )
  }

  static GetFirstHitPolyline(
    point: Point,
    rectangleNode: RectangleNode<Polyline, Point>,
  ): Polyline {
    const rectNode: RectangleNode<Polyline, Point> =
      InteractiveEdgeRouter.GetFirstHitRectangleNode(point, rectangleNode)
  ...
  static GetFirstHitRectangleNode(
    point: Point,
    rectangleNode: RectangleNode<Polyline, Point>,
  ...
  SetSourcePortAndSourceLoosePolyline(
    port: Port,
    sourceLoosePolylinePar: Polyline,
  ) {
  ...
  CalculateWholeVisibilityGraphOnExistingGraph() {
    ...
    if (this.UseSpanner) {
      visibilityGraphGenerator = new ConeSpanner(
        this.ObstacleCalculator.LooseObstacles,
        this.VisibilityGraph,
      )
    } else {
  ...
  RouteSplineFromPortToPortWhenTheWholeGraphIsReady(
    ...
    const reversed: boolean =
      (sourcePortLocal instanceof FloatingPort &&
        targetPortLocal instanceof CurvePort) ||
  ...
  GetEdgeGeomByRouting(
    ...
    if (curvePort != null) {
      ...
      if (this.targetPort instanceof CurvePort) {
        ...
        curve = this.RouteFromBoundaryPortToFloatingPort(
          this.targetLoosePolyline,
          smooth,
          t,
        )
      ...
      curve = this.RouteFromFloatingPortToFloatingPort(
        this.targetLoosePolyline,
        smooth,
        t,
      )
  ...
  RouteFromFloatingPortToAnywherePort(
    ...
    this._polyline = this.GetShortestPolylineToMulitpleTargets(
      this.SourceVisibilityVertex,
      Array.from(this.Targets(targetLoosePoly)),
    )
    if (this._polyline == null) {
      return null
    }
  ...
  BuildHook(port: HookUpAnywhereFromInsidePort) {
    ...
    // creating a hook
    const ellipse = Ellipse.mkFullEllipseNNP(
      port.HookSize,
      port.HookSize,
      this._polyline.end,
    )
    ...
    if (
      Point.getTriangleOrientation(
        intersections[0].x,
        this._polyline.end,
        this._polyline.endPoint.prev.point,
      ) == TriangleOrientation.Counterclockwise
    ) {
    ...
    const polylineTangent = this._polyline.end
      .sub(this._polyline.endPoint.prev.point)
      .normalize()
  ...
  ExtendPolyline(
    tangentAtIntersection: Point,
    x: IntersectionInfo,
    polylineTangent: Point,
    port: HookUpAnywhereFromInsidePort,
  ) {
  ...
  FixLastPolylinePointForAnywherePort(port: HookUpAnywhereFromInsidePort) {
    while (true) {
      const lastPointInside: PolylinePoint =
        this.GetLastPointInsideOfCurveOnPolyline(port.Curve)
      ...
      const rx = Curve.intersectionOne(
        port.Curve,
        LineSegment.mkPP(
          lastPointInside.point,
          lastPointInside.point.add(dir0),
        ),
        true,
      )
      const lx = Curve.intersectionOne(
        port.Curve,
        LineSegment.mkPP(
          lastPointInside.point,
          lastPointInside.point.add(dir1),
        ),
        true,
      )
      if (rx == null || lx == null) {
      ...
      const trimmedCurve =
        InteractiveEdgeRouter.GetTrimmedCurveForHookingUpAnywhere(
          port.Curve,
          lastPointInside,
          rx,
          lx,
        )
      const newLastPoint = trimmedCurve.value(
        trimmedCurve.closestParameter(lastPointInside.point),
      )
      if (
        !this.LineAvoidsTightHierarchyLPP(
          LineSegment.mkPP(lastPointInside.point, newLastPoint),
          this.SourceTightPolyline,
          null,
        )
      ) {
        const xx = Curve.intersectionOne(
          port.Curve,
          LineSegment.mkPP(lastPointInside.point, lastPointInside.next.point),
          false,
        )
        if (xx == null) {
          return
  ...
  static GetTrimmedCurveForHookingUpAnywhere(
    ...
    const clockwise =
      Point.getTriangleOrientation(x1.x, x0.x, lastPointInside.point) ==
      TriangleOrientation.Clockwise
  ...
  GetLastPointInsideOfCurveOnPolyline(curve: ICurve): PolylinePoint {
    for (let p = this._polyline.endPoint.prev; p != null; p = p.prev) {
      ...
      if (
        Curve.PointRelativeToCurveLocation(p.point, curve) ==
        PointLocation.Inside
      ) {
        return p
      }
  ...
  GetShortestPolylineToMulitpleTargets(
    sourceVisVertex: VisibilityVertex,
    ...
    const pathCalc =
      new SingleSourceMultipleTargetsShortestPathOnVisibilityGraph(
        sourceVisVertex,
        targets,
        this.VisibilityGraph,
      )
  ...
  Targets(targetLoosePoly: Polyline): Array<VisibilityVertex> {
    return Array.from(targetLoosePoly).map((p) =>
  ...
  ExtendVisibilityGraphFromFloatingSourcePort() {
    ...
    if (this.UseSpanner) {
      this._sourceVisibilityVertex = this.AddTransientVisibilityEdgesForPort(
        this.sourcePort.Location,
        this.SourceLoosePolyline,
      )
    } else {
      this._sourceVisibilityVertex =
        PointVisibilityCalculator.CalculatePointVisibilityGraph(
          Array.from(this.GetActivePolylines()).filter(
            (p) => p != this.SourceLoosePolyline,
          ),
          this.VisibilityGraph,
          this.StartPointOfEdgeRouting,
          VisibilityKind.Tangent,
