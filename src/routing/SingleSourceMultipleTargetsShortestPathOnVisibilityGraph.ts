...
export class SingleSourceMultipleTargetsShortestPathOnVisibilityGraph {
  ...
  constructor(
    ...
    targetVisVertices: Array<VisibilityVertex>,
  ...
  GetPath(): Array<VisibilityVertex> {
    ...
    while (!pq.IsEmpty()) {
      ...
      for (const e of this.current.OutEdges)
        if (this.PassableOutEdge(e)) this.ProcessNeighbor(pq, e, e.Target)
      for (const e of this.current.InEdges)
        if (this.PassableInEdge(e)) this.ProcessNeighbor(pq, e, e.Source)
    }
    return this._visGraph.PreviosVertex(this.current) == null
      ? null
      : this.CalculatePath()
  }
  ...
  private PassableOutEdge(e: VisibilityEdge): boolean {
    return (
      ...
      this.targets.has(e.Target) ||
      !SingleSourceMultipleTargetsShortestPathOnVisibilityGraph.IsForbidden(e)
    )
  }

  private PassableInEdge(e: VisibilityEdge): boolean {
    return (
      this.targets.has(e.Source) ||
