...
export class ActiveEdgeComparerWithRay implements IComparer<PolylinePoint> {
  pivot: Point
  ...
  Compare(x: PolylinePoint, y: PolylinePoint) {
    ...
    switch (
      Point.getTriangleOrientation(
        this.IntersectionOfTheRayAndInsertedEdge,
        y.point,
        y.nextOnPolyline.point,
      )
    ) {
  ...
  IntersectionPointBelongsToTheInsertedEdge(x: PolylinePoint): boolean {
    ...
    const b = x.nextOnPolyline.point.sub(
      this.IntersectionOfTheRayAndInsertedEdge,
    )
  ...
  IntersectEdgeWithRay(side: PolylinePoint, ray: Point): Point {
    return this.IntersectEdgeWithRayPPP(
      side.point,
