...
export class InteractiveTangentVisibilityGraphCalculator extends Algorithm {
  ...
  activeDiagonalComparer: ActiveDiagonalComparerWithRay =
    new ActiveDiagonalComparerWithRay()
  ...
  private AllocateDataStructures() {
    ...
    this.activeDiagonalTree = new RBTree<Diagonal>(
      this.activeDiagonalComparer.Compare.bind(this.activeDiagonalComparer),
    )
  }
  ...
  private AddVisibleEdge(t: Tangent) {
    VisibilityGraph.AddEdgeVV(
      getVertex(this.visibilityGraph, t.start),
      getVertex(this.visibilityGraph, t.End),
    )
  }
  ...
  private InitActiveDiagonals() {
    ...
    for (const diagonal of this.diagonals) {
      if (
        InteractiveTangentVisibilityGraphCalculator.RayIntersectDiagonal(
          firstTangentStart,
          firstTangentEnd,
          diagonal,
        )
        ...
        this.activeDiagonalComparer.PointOnTangentAndInsertedDiagonal =
          ActiveDiagonalComparerWithRay.IntersectDiagonalWithRay(
            firstTangentStart,
            firstTangentEnd,
            diagonal,
          )
      ...
      if (
        firstTangent.Diagonal.RbNode == this.activeDiagonalTree.treeMinimum()
      ) {
  ...
  private RemoveDiagonalFromActiveNodes(diag: Diagonal) {
    const changedNode: RBNode<Diagonal> = this.activeDiagonalTree.deleteSubTree(
      diag.RbNode,
    )
    if (changedNode != null) {
  ...
  private InsertActiveDiagonal(diagonal: Diagonal) {
    ...
    InteractiveTangentVisibilityGraphCalculator.MarkDiagonalAsActiveInTangents(
      diagonal,
    )
  }
  ...
  static RayIntersectDiagonal(
    ...
    return (
      Point.getTriangleOrientation(pivot, a, b) ==
        TriangleOrientation.Counterclockwise &&
      Point.getTriangleOrientation(pivot, pointOnRay, a) !=
        TriangleOrientation.Counterclockwise &&
      Point.getTriangleOrientation(pivot, pointOnRay, b) !=
  ...
  static TangentComparison(e0: Tangent, e1: Tangent): number {
    return StemStartPointComparer.CompareVectorsByAngleToXAxis(
      e0.End.point.sub(e0.start.point),
      e1.End.point.sub(e1.start.point),
    )
  }
  ...
  *AllObstacles(): IterableIterator<Polygon> {
  ...
  private OrganizeTangents() {
    ...
    this.tangents.sort(
      InteractiveTangentVisibilityGraphCalculator.TangentComparison,
    )
  }
  ...
  private ProcessPolygonQ(q: Polygon) {
    ...
    let couple = this.useLeftPTangents
      ? tangentPair.leftPLeftQ
      : tangentPair.rightPLeftQ
    const t0: Tangent = new Tangent(
      this.currentPolygon.pp(couple[0]),
      q.pp(couple[1]),
    )
    t0.IsLow = true
    t0.SeparatingPolygons = !this.useLeftPTangents
    couple = this.useLeftPTangents
      ? tangentPair.leftPRightQ
      : tangentPair.rightPRightQ
    const t1: Tangent = new Tangent(
      this.currentPolygon.pp(couple[0]),
      q.pp(couple[1]),
    )
    t1.IsLow = false
