...
export class CdtSweeper extends Algorithm {
  ...
  constructor(
    ...
    this.front.insert(
      new CdtFrontElement(this.listOfSites[0], firstTriangle.Edges[1]),
    )
    this.p_1 = this.p_1
    this.p_2 = this.p_2
  ...
  static RemoveTriangleWithEdges(
    cdtTriangles: Set<CdtTriangle>,
    t: CdtTriangle,
  ) {
  ...
  static RemoveTriangleButLeaveEdges(
    cdtTriangles: Set<CdtTriangle>,
    t: CdtTriangle,
  ) {
    cdtTriangles.delete(t)
  ...
  EdgeEvent(edge: CdtEdge) {
    ...
    const edgeInserter = new EdgeInserter(
      edge,
      this.triangles,
      this.front,
  ...
  LeftCase(
    pi: CdtSite,
    hittedFrontElementNode: RBNode<CdtFrontElement>,
    t: {rightSite: CdtSite},
  ): CdtSite {
    // left case
  ...
  ShortcutTwoFrontElements(
    aNode: RBNode<CdtFrontElement>,
    bNode: RBNode<CdtFrontElement>,
  ...
  TryTriangulateBasinToTheLeft(leftLegNode: RBNode<CdtFrontElement>) {
    ...
    while (true) {
      ...
      if (
        ...
        leftLegNode.item.LeftSite.point.y > leftLegNode.item.RightSite.point.y
      ) {
        stack.push(prev.item.LeftSite)
      } else {
  ...
  static FindNodeInFrontBySite(
    cdtFrontElements: RBTree<CdtFrontElement>,
    piSite: CdtSite,
  ): RBNode<CdtFrontElement> {
    return cdtFrontElements.findLast(
      (x) => x.LeftSite.point.x <= piSite.point.x,
    )
  }
  ...
  InsertAndLegalizeTriangle(pi: CdtSite, frontElement: CdtFrontElement) {
    if (
      ...
      t = CdtTriangle.mkSSSD(
        frontElement.LeftSite,
        oppositeSite,
        pi,
        this.createEdgeDelegate,
      )
      const t1 = CdtTriangle.mkSSSD(
        frontElement.RightSite,
        oppositeSite,
        pi,
        this.createEdgeDelegate,
      )
...
export function InCircle(
  ...
  return (
    axdx * (bydy * t2 - cydy * t1) -
      bxdx * (aydy * t2 - cydy * t0) +
      cxdx * (aydy * t1 - bydy * t0) >
...
function Flip(pi: CdtSite, edge: CdtEdge): CdtEdge {
  ...
  const edgeBeforPi = t.Edges[eIndex + 1]
  const edgeBeforPl = ot.Edges[eOtherIndex + 1]
