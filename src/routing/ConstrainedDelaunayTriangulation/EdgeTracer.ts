...
export class EdgeTracer {
  ...
  ProcessLeftFrontPiercedElement() {
    ...
    do {
      ...
      Point.pointToTheLeftOfLine(
        v.item.LeftSite.point,
        this.a.point,
        this.b.point,
      )
    ) //that is why we are adding to the left polygon
  ...
  FindPiercedTriangle(v: RBNode<CdtFrontElement>) {
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      const signedArea1 = RealNumberSpan.sign(
        Point.signedDoubledTriangleArea(
          ei.upperSite.point,
          this.a.point,
          this.b.point,
  ...
  ProcessRightFrontPiercedElement() {
    ...
    do {
      ...
      Point.pointToTheRightOfLine(
        v.item.RightSite.point,
        this.a.point,
        this.b.point,
      )
    ) //that is why we are adding to the right polygon
    this.elementsToBeRemovedFromFront.push(v.item)
  ...
  BIsReached(): boolean {
    const node =
      this.piercedToTheLeftFrontElemNode ?? this.piercedToTheRightFrontElemNode
    if (node != null) {
  ...
  Init() {
    ...
    // new[] {new LineSegment(piercedEdge.upperSite.point, piercedEdge.lowerSite.point)});
    const frontElemNodeRightOfA = CdtSweeper.FindNodeInFrontBySite(
      this.front,
      this.a,
    )
    ...
    if (
      Point.pointToTheLeftOfLine(
        this.b.point,
        frontElemNodeLeftOfA.item.LeftSite.point,
        frontElemNodeLeftOfA.item.RightSite.point,
      )
    ) {
      ...
      Point.pointToTheRightOfLine(
        this.b.point,
        frontElemNodeRightOfA.item.RightSite.point,
        frontElemNodeRightOfA.item.LeftSite.point,
      )
    ) {
