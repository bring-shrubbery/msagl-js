...
export class ConeSpanner extends Algorithm {
  ...
  static mk(
    ...
    Assert.assert(
      borderPolyline == null ||
        obstacles.every((o) => Curve.CurveIsInsideOther(o, borderPolyline)),
    )
    Assert.assert(
      borderPolyline == null ||
  ...
  run() {
    ...
    if (!this.Bidirectional) {
      let angle: number
      for (let i = 0; this.coneAngle * i <= offset; i++) {
        super.ProgressStep()
        this.AddDirection(
          new Point(Math.cos(angle), Math.sin(angle)),
          this.BorderPolyline,
          this._visibilityGraph,
        )
      }
    } else {
  ...
  HandleBideractionalCase() {
    ...
    for (let i = 0; i < k; i++) {
      ...
      this.AddDirection(
        new Point(Math.cos(angle) * -1, Math.sin(angle) * -1),
        this.BorderPolyline,
        vg1,
      )
  ...
  AddIntersectionOfBothDirectionSweepsToTheResult(
    vg0: VisibilityGraph,
    vg1: VisibilityGraph,
  ) {
