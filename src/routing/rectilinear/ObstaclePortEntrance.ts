...
export class ObstaclePortEntrance {
  ...
  constructor(
    ...
    const lineSeg = LineSegment.mkPP(
      ...
      StaticGraphUtility.RectangleBorderIntersect(
        oport.Obstacle.VisibilityBoundingBox,
        this.UnpaddedBorderIntersect,
        outDir,
      ),
    )
    ...
    if (
      ...
      (this.Obstacle.IsGroup && !this.Obstacle.IsInConvexHull)
    ) {
  ...
  HasGroupCrossingBeforePoint(point: Point): boolean {
    ...
    return (
      PointComparer.GetDirections(
        this.MaxVisibilitySegment.start,
        pac.Location,
      ) == PointComparer.GetDirections(pac.Location, point)
    )
  }
  ...
  AddToAdjacentVertex(
    ...
    let borderVertex: VisibilityVertex = transUtil.VisGraph.FindVertex(
      this.VisibilityBorderIntersect,
    )
  ...
  ExtendEdgeChain(
    ...
    const unpaddedBorderVertex = transUtil.FindOrAddVertex(
      this.UnpaddedBorderIntersect,
    )
    transUtil.FindOrAddEdge(
      unpaddedBorderVertex,
      paddedBorderVertex,
      this.unpaddedToPaddedBorderWeight,
    )
    if (routeToCenter) {
      //  Link the CenterVertex to the vertex at UnpaddedBorderIntersect.
      transUtil.ConnectVertexToTargetVertex(
        this.ObstaclePort.CenterVertex,
        unpaddedBorderVertex,
        this.OutwardDirection,
        this.InitialWeight,
      )
