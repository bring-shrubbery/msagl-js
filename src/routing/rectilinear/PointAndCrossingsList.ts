...
export class PointAndCrossingsList {
  ...
  Add(intersect: Point, crossings: Array<GroupBoundaryCrossing>) {
    this.ListOfPointsAndCrossings.push(
      new PointAndCrossings(intersect, crossings),
    )
  }
  ...
  CurrentIsBeforeOrAt(comparand: Point): boolean {
    ...
    return (
      PointComparer.ComparePP(
        this.ListOfPointsAndCrossings[this.index].Location,
        comparand,
      ) <= 0
    )
  }
  ...
  MergeFrom(other: PointAndCrossingsList) {
    ...
    let otherIndex = 0
    const newCrossingsList = new Array<PointAndCrossings>(
  ...
  Trim(start: Point, end: Point) {
    this.Reset()
    if (
      this.ListOfPointsAndCrossings == null ||
      0 == this.ListOfPointsAndCrossings.length
    ) {
      return
    }
    ...
    this.ListOfPointsAndCrossings = this.ListOfPointsAndCrossings.filter(
      (pair) =>
        PointComparer.ComparePP(pair.Location, start) >= 0 &&
        PointComparer.ComparePP(pair.Location, end) <= 0,
    )
  }
