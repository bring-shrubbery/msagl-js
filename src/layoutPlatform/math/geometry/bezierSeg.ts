...
export class BezierSeg implements ICurve {
  ...
  get length() {
    return BezierSeg.lengthOnControlPolygon(
      this.b[0],
      this.b[1],
      this.b[2],
      this.b[3],
  ...
  static lengthOnControlPolygon(
    ...
    if (
      ...
      return (
        BezierSeg.lengthOnControlPolygon(b0, mb0, mmb0, mmmb0) +
        BezierSeg.lengthOnControlPolygon(mmmb0, mmb1, mb2, b3)
      )
    }
  ...
  closestParameterWithinBounds(targetPoint: Point, low: number, high: number) {
    ...
    return ClosestPointOnCurve.closestPoint(
      this,
      targetPoint,
      closest,
      low,
  ...
  Gpp(t: number) {
    ...
    return (
      3 * ((v * v) / u + u * (xpp * xpp + xp * xppp + ypp * ypp + yp * yppp))
    )
  }
  ...
  evaluateError(length: number, t: number) {
    ...
    const lengthAtT = BezierSeg.lengthOnControlPolygon(
      this.b[0],
      mb0,
      mmb0,
      mmmb0,
