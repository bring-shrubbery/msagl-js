...
export class Curve implements ICurve {
  ...
  public static PointRelativeToCurveLocation(
    point: Point,
    curve: ICurve,
  ): PointLocation {
    ...
    for (let i = 13; i < 360; i += 13) {
      ...
      const intersections = this.getAllIntersectionsOfLineAndICurve(
        ls,
        curve,
        true,
      )
  ...
  static AllIntersectionsAreGood(
...
*/
  ...
  static NonIntersectingCurveIsInsideOther(a: ICurve, b: ICurve): boolean {
    ...
    for (let par: number = a.parStart; par < a.parEnd; par += 0.5) {
      ...
      const parLoc: PointLocation = Curve.PointRelativeToCurveLocation(
        a.value(par),
        b,
      )
