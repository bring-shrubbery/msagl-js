...
export class Qpsc {
  ...
  AddVariable(variable: Variable) {
    ...
    this.vectorWiDi[variable.Ordinal] =
      2 * (variable.Weight * variable.DesiredPos) * -1
    ...
    if (null != variable.Neighbors) {
      for (const neighborWeightPair of variable.Neighbors) {
        ...
        //  Per above comments:
        //      First the diagonal entries: A[i][i] = wi + Sum(wij for every neighbor j of i).
        this.vectorPrevY[variable.Ordinal] =
    ...
    for (let rowNum = 0; rowNum < this.matrixQ.length; rowNum++) {
      ...
      for (let sparseCol = 0; sparseCol < row.length; sparseCol++) {
        if (row[sparseCol].Column == rowNum) {
          ...
          row[sparseCol].Value =
            row[sparseCol].Value *
            (this.vectorQpscVars[rowNum].Variable.Scale *
  ...
  PreProject(): boolean {
    ...
    //
    const alphaNumerator: number = Qpsc.VectorVectorMultiply(
      this.gradientVector,
      this.gradientVector,
    )
    ...
    if (0 != alphaNumerator) {
      ...
      alphaDenominator = Qpsc.VectorVectorMultiply(
        this.vectorQg,
        this.gradientVector,
      )
    }
    ...
    Qpsc.VectorScaledVectorSubtract(
      this.vectorPrevY,
      alpha,
      this.gradientVector,
      this.vectorCurY,
  ...
  PostProject(): boolean {
    ...
    //
    const betaNumerator: number = Qpsc.VectorVectorMultiply(
      this.gradientVector,
      this.vectorCurY,
    )
    //  Compute numerator of stepsize
    ...
    if (0 != betaNumerator) {
      ...
      //  Now p#(Qp).
      const betaDenominator: number = Qpsc.VectorVectorMultiply(
        this.vectorQg,
        this.vectorCurY,
      )
  ...
  private GetFunctionValue(positions: number[]): number {
    //  (x#Ax)/2...
    const value: number =
      Qpsc.VectorVectorMultiply(this.gradientVector, positions) / 2
    //  (x'Ax)/2 + bx...
  ...
  private static VectorVectorAdd(
    lhs: number[],
    rhs: number[],
    result: number[],
  ) {
  ...
  private static VectorVectorSubtract(
    lhs: number[],
    rhs: number[],
    result: number[],
  ) {
  ...
  private static VectorScaledVectorSubtract(
    lhs: number[],
    scale: number,
    rhs: number[],
    result: number[],
