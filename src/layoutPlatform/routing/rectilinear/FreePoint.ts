    ...
    export class FreePoint {
        ...
        //  extension because we may be on the edge so can't calculate the direction.
         AddEdgeToAdjacentEdge(transUtil: TransientGraphUtility, targetEdge: VisibilityEdge, dirToExtend: Direction, limitRect: Rectangle): VisibilityVertex {
            let targetIntersect: Point = StaticGraphUtility.SegmentIntersectionEP(targetEdge, this.Point);
         ...
         ExtendEdgeChain(transUtil: TransientGraphUtility, targetVertex: VisibilityVertex, dirToExtend: Direction, limitRect: Rectangle) {
            ...
            let segmentAndCrossings: SegmentAndCrossings = this.GetSegmentAndCrossings(this.IsOverlapped ? targetVertex : this.Vertex, dirToExtend, transUtil);
           
            transUtil.ExtendEdgeChainVRLPB(targetVertex, limitRect, segmentAndCrossings[0], segmentAndCrossings[1], extendOverlapped);
        }
        
        private GetSegmentAndCrossings(startVertex: VisibilityVertex, dirToExtend: Direction, transUtil: TransientGraphUtility): SegmentAndCrossings {
            let dirIndex = CompassVector.ToIndex(dirToExtend);
            ...
            if ((segmentAndCrossings == null)) {
                ...
                let maxVisibilitySegment = transUtil.ObstacleTree.CreateMaxVisibilitySegment(startVertex.point, dirToExtend, t);
         ...
         MaxVisibilityInDirectionForNonOverlappedFreePoint(dirToExtend: Direction, transUtil: TransientGraphUtility): Point {
            Assert.assert(!this.IsOverlapped, "Do not precalculate overlapped obstacle visibility as we should extend from the outer target vertex instead");
            let segmentAndCrossings: SegmentAndCrossings = this.GetSegmentAndCrossings(this.Vertex, dirToExtend, transUtil);
            return segmentAndCrossings[0].end;
        }
        
         AddOobEdgesFromGraphCorner(transUtil: TransientGraphUtility, cornerPoint: Point) {
            let dirs: Direction = PointComparer.GetDirections(cornerPoint, this.Vertex.point);
            let cornerVertex: VisibilityVertex = transUtil.VisGraph.FindVertex(cornerPoint);
            //  For waypoints we want to be able to enter in both directions. 
            transUtil.ConnectVertexToTargetVertex(cornerVertex, this.Vertex, (dirs 
                            & (Direction.North | Direction.South)), ScanSegment.NormalWeight);
            transUtil.ConnectVertexToTargetVertex(cornerVertex, this.Vertex, (dirs 
                            & (Direction.East | Direction.West)), ScanSegment.NormalWeight);
        }
