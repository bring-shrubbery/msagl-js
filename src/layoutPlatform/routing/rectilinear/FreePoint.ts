...
export class FreePoint {
  ...
  private maxVisibilitySegmentsAndCrossings: SegmentAndCrossings[] = new Array(
    4,
  )
  ...
  AddEdgeToAdjacentEdge(
    ...
    const targetIntersect: Point = StaticGraphUtility.SegmentIntersectionEP(
      targetEdge,
      this.Point,
    )
    ...
    if (null != targetVertex) {
      ...
      targetVertex = transUtil.AddEdgeToTargetEdge(
        this.Vertex,
        targetEdge,
        targetIntersect,
      )
  ...
  ExtendEdgeChain(
    ...
    if (extendOverlapped) {
      ...
      extendOverlapped = transUtil.ObstacleTree.PointIsInsideAnObstaclePD(
        targetVertex.point,
        dirToExtend,
      )
    }
    ...
    transUtil.ExtendEdgeChainVRLPB(
      targetVertex,
      limitRect,
      segmentAndCrossings[0],
      segmentAndCrossings[1],
      extendOverlapped,
    )
  }
  ...
  MaxVisibilityInDirectionForNonOverlappedFreePoint(
    dirToExtend: Direction,
    transUtil: TransientGraphUtility,
  ): Point {
    /*Assert.assert(
  ...
  AddOobEdgesFromGraphCorner(
    transUtil: TransientGraphUtility,
    cornerPoint: Point,
  ) {
    const dirs: Direction = PointComparer.GetDirections(
      cornerPoint,
      this.Vertex.point,
    )
