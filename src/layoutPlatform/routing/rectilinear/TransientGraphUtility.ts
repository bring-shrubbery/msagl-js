...
export class TransientGraphUtility {
  ...
  FindOrAddEdge(
    ...
    const dirToTarget: Direction = PointComparer.GetPureDirectionVV(
      sourceVertex,
      targetVertex,
    )
  ...
  static FindBracketingVertices(
    ...
    t.bracketSource = sourceVertex
    for (;;) {
      t.bracketTarget = StaticGraphUtility.FindAdjacentVertex(
        t.bracketSource,
        dirToTarget,
      )
      ...
      if (
        dirToTarget !=
        PointComparer.GetDirections(t.bracketTarget.point, targetPoint)
      ) {
  ...
  FindPerpendicularOrContainingEdge(
    ...
    while (true) {
      ...
      const dirCheck: Direction = PointComparer.GetDirections(
        nextVertex.point,
        pointLocation,
      )
  ...
  FindNearestPerpendicularOrContainingEdge(
    startVertex: VisibilityVertex,
    dir: Direction,
    pointLocation: Point,
  ): VisibilityEdge {
    ...
    while (Direction.None != currentDirTowardLocation) {
      ...
      if (
        0 !=
        (CompassVector.OppositeDir(dirTowardLocation) &
    ...
    while (true) {
      perpEdge = this.FindPerpendicularOrContainingEdge(
        currentVertex,
        dir,
        pointLocation,
      )
      ...
      currentVertex = StaticGraphUtility.FindAdjacentVertex(
        currentVertex,
        CompassVector.OppositeDir(dirTowardLocation),
      )
    }
  ...
  ConnectVertexToTargetVertex(
    sourceVertex: VisibilityVertex,
    targetVertex: VisibilityVertex,
    finalEdgeDir: Direction,
    weight: number,
  ) {
    ...
    const targetDirs: Direction = PointComparer.GetDirections(
      sourceVertex.point,
      targetVertex.point,
    )
    ...
    const bendPoint: Point = StaticGraphUtility.FindBendPointBetween(
      sourceVertex.point,
      targetVertex.point,
      finalEdgeDir,
    )
  ...
  SplitEdge(
    ...
    if (
      Point.closeDistEps(edge.Source.point, splitVertex.point) ||
      Point.closeDistEps(edge.Target.point, splitVertex.point)
    ) {
      //  No split needed.
      return edge
    }
    ...
    if (
      (this.IsSparseVg || edge.Weight == ScanSegment.OverlappedWeight) &&
  ...
  ExtendEdgeChainVRLPB(
    ...
    const dir = PointComparer.GetDirections(
      maxVisibilitySegment.start,
      maxVisibilitySegment.end,
    )
    ...
    const oppositeFarBound: number = StaticGraphUtility.GetRectangleBound(
      limitRect,
      dir,
    )
    const maxDesiredSplicePoint: Point = StaticGraphUtility.IsVerticalD(dir)
      ? GeomConstants.RoundPoint(
          new Point(startVertex.point.x, oppositeFarBound),
        )
      : GeomConstants.RoundPoint(
          new Point(oppositeFarBound, startVertex.point.y),
        )
    ...
    if (
      PointComparer.GetDirections(startVertex.point, maxDesiredSplicePoint) !=
      dir
    ) {
    ...
    if (
      PointComparer.GetDirections(
        maxDesiredSplicePoint,
        maxDesiredSegment.end,
      ) == dir
    ) {
      maxDesiredSegment = LineSegment.mkPP(
        maxDesiredSegment.start,
        maxDesiredSplicePoint,
      )
    }
  ...
  ExtendEdgeChain(
    ...
    const segmentDir: Direction = PointComparer.GetDirections(
      startVertex.point,
      maxDesiredSegment.end,
    )
    ...
    let spliceSource: VisibilityVertex = StaticGraphUtility.FindAdjacentVertex(
      startVertex,
      spliceSourceDir,
    )
    if (spliceSource == null) {
      spliceSourceDir = CompassVector.OppositeDir(spliceSourceDir)
      spliceSource = StaticGraphUtility.FindAdjacentVertex(
        startVertex,
        spliceSourceDir,
      )
      if (spliceSource == null) {
  ...
  private SpliceGroupBoundaryCrossings(
    crossingList: PointAndCrossingsList,
    startVertex: VisibilityVertex,
    ...
    startVertex = TransientGraphUtility.TraverseToFirstVertexAtOrAbove(
      startVertex,
      start,
    ...
    for (
      ...
      const isFinalVertex: boolean =
        PointComparer.ComparePP(currentVertex.point, end) >= 0
      while (crossingList.CurrentIsBeforeOrAt(currentVertex.point)) {
        ...
        if (PointComparer.ComparePP(pac.Location, startVertex.point) > 0) {
          if (PointComparer.ComparePP(pac.Location, end) <= 0) {
            this.SpliceGroupBoundaryCrossing(
              currentVertex,
              pac,
              CompassVector.OppositeDir(dir),
            )
  ...
  private static TraverseToFirstVertexAtOrAbove(
    ...
    for (;;) {
      const nextVertex = StaticGraphUtility.FindAdjacentVertex(
        returnVertex,
        dir,
      )
  ...
  private SpliceGroupBoundaryCrossing(
    currentVertex: VisibilityVertex,
  ...
  ExtendSpliceWorker(
    ...
    for (;;) {
      ...
      if (
        TransientGraphUtility.IsPointPastSegmentEnd(
          maxVisibilitySegment,
          nextExtendPoint,
        )
      ) {
      ...
      if (isOverlapped) {
        isOverlapped = this.SeeIfSpliceIsStillOverlapped(
          extendDir,
          nextExtendVertex,
        )
      }
  ...
  private static GetNextSpliceSource(
    ...
    if (nextSpliceSource == null) {
      ...
      for (;;) {
        ...
        const nextSpliceSourceExtend = StaticGraphUtility.FindAdjacentVertex(
          nextSpliceSource,
          extendDir,
        )
  ...
  private SeeIfSpliceIsStillOverlapped(
    extendDir: Direction,
    nextExtendVertex: VisibilityVertex,
  ): boolean {
    ...
    let edge = this.FindNextEdge(
      nextExtendVertex,
      CompassVector.RotateLeft(extendDir),
    )
    let maybeFreeSpace =
      edge == null ? false : ScanSegment.NormalWeight == edge.Weight
    if (!maybeFreeSpace) {
      edge = this.FindNextEdge(
        nextExtendVertex,
        CompassVector.RotateRight(extendDir),
      )
      maybeFreeSpace =
        edge == null ? false : ScanSegment.NormalWeight == edge.Weight
    }
    ...
    return (
      !maybeFreeSpace ||
      this.ObstacleTree.PointIsInsideAnObstaclePD(
        nextExtendVertex.point,
        extendDir,
      )
    )
  }

  IsSkippableSpliceSourceWithNullSpliceTarget(
    spliceSource: VisibilityVertex,
    extendDir: Direction,
  ): boolean {
    if (
      TransientGraphUtility.IsSkippableSpliceSourceEdgeWithNullTarget(
        StaticGraphUtility.FindAdjacentEdge(spliceSource, extendDir),
      )
    ) {
      return true
    }

    const spliceSourceEdge = StaticGraphUtility.FindAdjacentEdge(
      spliceSource,
      CompassVector.OppositeDir(extendDir),
    )
  ...
  static IsSkippableSpliceSourceEdgeWithNullTarget(
    spliceSourceEdge: VisibilityEdge,
  ): boolean {
    return (
  ...
  static IsPointPastSegmentEnd(maxSegment: LineSegment, point: Point): boolean {
    return (
      PointComparer.GetDirections(maxSegment.start, maxSegment.end) ==
  ...
  toString(): string {
    return String.Format(
      '{0} {1}',
      this.AddedVertices.length,
      this.edgesToRestore.length,
    )
  }
}
