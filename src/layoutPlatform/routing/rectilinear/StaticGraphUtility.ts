...
export class StaticGraphUtility {
  ...
  static FindBendPointBetween(
    ...
    return !StaticGraphUtility.IsVerticalD(finalEdgeDir)
      ? new Point(sourcePoint.x, targetPoint.y)
      : new Point(targetPoint.x, sourcePoint.y)
  }

  static SegmentIntersectionPPP(
    first: Point,
    second: Point,
    from: Point,
  ): Point {
  ...
  static IntervalsOverlapPPPP(
    ...
    return (
      ...
      PointComparer.ComparePP(start1, end2) !=
        PointComparer.ComparePP(end1, start2)
    )
  }

  static IntervalsAreCollinear(
    start1: Point,
    end1: Point,
    start2: Point,
    end2: Point,
  ...
  static IntervalsAreSame(
    ...
    return (
      PointComparer.EqualPP(start1, start2) && PointComparer.EqualPP(end1, end2)
    )
  }
  ...
  static IntervalsIntersect(
    ...
    return StaticGraphUtility.PointIsOnSegmentPPP(
      firstStart,
      firstEnd,
      intersect,
    ) &&
  ...
  static SegmentIntersectionEP(edge: VisibilityEdge, from: Point): Point {
    return StaticGraphUtility.SegmentIntersectionPPP(
      edge.SourcePoint,
      edge.TargetPoint,
      from,
    )
  ...
  static Slope(start: Point, end: Point, scanDir: ScanDirection): number {
    ...
    return (
      lineDir.dot(scanDir.PerpDirectionAsPoint) /
      lineDir.dot(scanDir.DirectionAsPoint)
    )
  }
  ...
  static RectangleBorderIntersect(
    ...
    switch (dir) {
      ...
      case Direction.South:
        return new Point(
          point.x,
          StaticGraphUtility.GetRectangleBound(boundingBox, dir),
        )
        break
      case Direction.East:
      case Direction.West:
