...
export class SsstRectilinearPath {
  ...
  private InitPath(
    ...
    this.Source = source
    const cost: number =
      this.TotalCostFromSourceToVertex(0, 0) +
      this.HeuristicDistanceFromVertexToTarget(source.point, Direction.None)
  ...
  private InitEntryDirectionsAtTarget(vert: VisibilityVertex): boolean {
    ...
    for (const edge of vert.OutEdges) {
      this.EntryDirectionsToTarget =
        this.EntryDirectionsToTarget |
        CompassVector.DirectionFromPointToPoint(edge.TargetPoint, vert.point)
    }

    for (const edge of vert.InEdges) {
      this.EntryDirectionsToTarget =
        this.EntryDirectionsToTarget |
        CompassVector.DirectionFromPointToPoint(edge.SourcePoint, vert.point)
    }
  ...
  private HeuristicDistanceFromVertexToTarget(
    ...
    if (
      closeDistEps(vectorToTarget.x, 0) &&
      closeDistEps(vectorToTarget.y, 0)
    ) {
    ...
    return (
      this.CombinedCost(
        SsstRectilinearPath.ManhattanDistance(point, this.Target.point),
        numberOfBends,
      ) + this.targetCostAdjustment
    )
  }
  ...
  private GetNumberOfBendsForPureDirection(
    ...
    if ((dirToTarget & entryDirToVertex) == dirToTarget) {
      if (
        SsstRectilinearPath.IsInDirs(dirToTarget, this.EntryDirectionsToTarget)
      ) {
        return 0
      }

      if (
        SsstRectilinearPath.IsInDirs(
          SsstRectilinearPath.Left(dirToTarget),
          this.EntryDirectionsToTarget,
        ) ||
        SsstRectilinearPath.IsInDirs(
          SsstRectilinearPath.Right(dirToTarget),
          this.EntryDirectionsToTarget,
        )
      ) {
        return 2
    ...
    return (
      this.GetNumberOfBendsForPureDirection(
  ...
  private QueueReversedEntryToNeighborVertexIfNeeded(
    ...
    if (
      this.CombinedCost(t.length, t.numberOfBends) <
        this.CombinedCost(
          entryFromNeighbor.Length,
          entryFromNeighbor.NumberOfBends,
        ) ||
  ...
  GetPathWithCost(
    ...
    while (this.queue.count > 0) {
      ...
      if (bestVertex == this.Target) {
        ...
        this.upperBoundOnCost = Math.min(
          this.MultistageAdjustedCostBound(bestEntry.Cost),
          this.upperBoundOnCost,
        )
        continue
      }
      ...
      this.ExtendPathAlongInEdges(
        bestEntry,
        bestVertex.InEdges,
        preferredBendDir,
      )
      this.ExtendPathAlongOutEdges(
        bestEntry,
        bestVertex.OutEdges,
        preferredBendDir,
      )
  ...
  private ExtendPathAlongEdge(
    ...
    if (neigVer == bestEntry.PreviousVertex) {
      ...
      this.ExtendPathToNeighborVertex(
        bestEntry,
        <VisibilityVertexRectilinear>neigVer,
        edge.Weight,
      )
      return
    }
    ...
    const neigDir = CompassVector.DirectionFromPointToPoint(
      bestEntry.Vertex.point,
      neigVer.point,
    )
  ...
  private ExtendPathToNeighborVertex(
    ...
    const dirToNeighbor = CompassVector.DirectionFromPointToPoint(
      bestEntry.Vertex.point,
      neigVer.point,
    )
    const neigEntry =
  ...
  private CreateAndEnqueueReversedEntryToNeighborVertex(
    ...
    if (bestEntry.Vertex.VertexEntries != null) {
      const dirFromNeighbor = CompassVector.DirectionFromPointToPoint(
        neigVer.point,
        bestEntry.Vertex.point,
      )
