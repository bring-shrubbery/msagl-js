...
export class ScanSegmentVector {
  ...
  constructor(coordMap: Set<number>, isHorizontal: boolean) {
    ...
    const coords = Array.from(coordMap).sort((a, b) =>
      a > b ? 1 : a < b ? -1 : 0,
    )
  ...
  CreateScanSegment(
    ...
    this.CurrentSlot.AppendScanSegment(
      new ScanSegment(start, end, weight, gbcList),
    )
  }
  ...
  ConnectAdjoiningSegmentEndpoints() {
    ...
    for (const item of this.vector) {
      ...
      for (
        let segment = prevSegment.NextSegment;
        segment != null;
        segment = segment.NextSegment
      ) {
        if (
          segment.HasSparsePerpendicularCoords &&
  ...
  toString(): string {
    return (
      (this.IsHorizontal ? '(H) count' : '(V) count == ') + this.vector.length
    )
  }
}
