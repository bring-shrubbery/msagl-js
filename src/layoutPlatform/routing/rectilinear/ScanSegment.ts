...
import { StaticGraphUtility } from "./StaticGraphUtility"
 ...
 //  This is a single segment added by the ScanLine.
    ...
    private startPoint: Point;
     ...
     readonly static NormalWeight: number = VisibilityEdge.DefaultWeight;
    
     readonly static ReflectionWeight: number = 5;
    
     readonly static OverlappedWeight: number = 500;
    ...
    //  ctor
     Update(start: Point, end: Point) {
        Assert.assert((PointComparer.EqualPP(start, end) || StaticGraphUtility.IsAscending(PointComparer.GetPureDirectionPP(start, end))), "non-ascending segment");
 ...
 AppendVisibilityVertex(vg: VisibilityGraph, newVertex: VisibilityVertex) {
        ...
        Assert.assert(((this.LowestVisibilityVertex == null) 
                        == (this.HighestVisibilityVertex == null)), "Mismatched null Lowest/HighestVisibilityVertex");
        Assert.assert(StaticGraphUtility.PointIsOnSegmentSP(this, newVertex.point), "newVertex is out of segment range");
        if ((this.HighestVisibilityVertex == null)) {
            ...
            if (PointComparer.IsPureLower(newVertex.point, this.HighestVisibilityVertex.point)) {
                Assert.assert((null != vg.FindEdgePP(newVertex.point, this.HighestVisibilityVertex.point)), "unexpected low/middle insertion to ScanSegment");
                return;
            }
    ...
    private AddVisibilityEdge(source: VisibilityVertex, target: VisibilityVertex): VisibilityEdge {
        ...
        Assert.assert(PointComparer.IsPureLower(source.point, target.point), "Impure or reversed direction encountered");
        //  Make sure we aren't adding two edges in the same direction to the same vertex.
        Assert.assert(StaticGraphUtility.FindNextVertex(source, (StaticGraphUtility.EdgeDirectionVV(source, target))) == null, "Duplicate outEdge from Source vertex");
        Assert.assert(StaticGraphUtility.FindNextVertex(target, (StaticGraphUtility.EdgeDirectionVV(target, source))) == null, "Duplicate inEdge to Target vertex");
        let edge = new VisibilityEdge(source, target, this.Weight);
    ...
    private LoadStartOverlapVertexIfNeeded(vg: VisibilityGraph) {
        ...
        if (this.NeedStartOverlapVertex) {
            let vertex: VisibilityVertex = vg.FindVertex(this.Start);
    ...
    private LoadEndOverlapVertexIfNeeded(vg: VisibilityGraph) {
        ...
        if (this.NeedEndOverlapVertex) {
            let vertex: VisibilityVertex = vg.FindVertex(this.End);
 ...
 IntersectsSegment(seg: ScanSegment): boolean {
    ...
    private sparsePerpendicularCoords: Set<number>;
     ...
     get HasSparsePerpendicularCoords(): boolean {
        ...
        (this.sparsePerpendicularCoords.Count > 0);
    }
    
    private CreatePointFromPerpCoord(perpCoord: number): Point {
        return new Point(this.Start.x, perpCoord);
        // TODO: Warning!!!, inline IF is not supported ?
        this.IsVertical;
     ...
     AddSparseVertexCoord(perpCoord: number) {
        Assert.assert(this.ContainsPoint(this.CreatePointFromPerpCoord(perpCoord)), "vertexLocation is not on Segment");
     ...
     CreateSparseVerticesAndEdges(vg: VisibilityGraph) {
        ...
        for (let perpCoord in this.sparsePerpendicularCoords.OrderBy(() => {  }, d)) {
            let vertexLocation = this.CreatePointFromPerpCoord(perpCoord);
            Assert.assert(this.ContainsPoint(vertexLocation), "vertexLocation is not on Segment");
            this.AppendVisibilityVertex(vg, vg.FindVertex(vertexLocationUnknown, Question, Question, vg.AddVertexP(vertexLocation));
        }
    ...
    private AppendGroupCrossingsThroughPoint(vg: VisibilityGraph, lastPoint: Point): boolean {
        ...
        while (this.GroupBoundaryPointAndCrossingsList.CurrentIsBeforeOrAt(lastPoint)) {
            ...
            let pac: PointAndCrossings = this.GroupBoundaryPointAndCrossingsList.Pop();
            ...
            if ((PointComparer.Compare(pac.Location, this.Start) > 0)) {
                lowDirCrossings = PointAndCrossingsList.ToCrossingArray(pac.Crossings, ScanDirection.OppositeDirection);
            }
            ...
            if (((null != lowDirCrossings) || (null != highDirCrossings))) {
                ...
                if ((this.LowestVisibilityVertex == null)) {
                    ...
                    Assert.assert(PointComparer.Equal(this.End, crossingVertex.point), "Expected this.End crossingVertex");
    ...
    private static GetCrossingInteriorVertex(vg: VisibilityGraph, crossingVertex: VisibilityVertex, crossing: GroupBoundaryCrossing): VisibilityVertex {
        let interiorPoint: Point = crossing.GetInteriorVertexPoint(crossingVertex.point);
        return;
        vg.AddVertexP(interiorPoint);
    }
    
    private AddCrossingEdge(vg: VisibilityGraph, lowVertex: VisibilityVertex, highVertex: VisibilityVertex, crossings: GroupBoundaryCrossing[]) {
        ...
        if ((null != this.HighestVisibilityVertex)) {
            ...
            if (PointComparer.Equal(this.HighestVisibilityVertex.point, highVertex.point)) {
                ...
                Assert.assert((edge != null), "Inconsistent forward-backward sequencing in HighVisibilityVertex");
            }
            else {
        ...
        let prevIsPassable = edge.IsPassable;
        if ((prevIsPassable == null)) {
            ...
            return (crossingsArray.Any(() => {  }, s.IsTransparent) || prevIsPassable());
    ...
    private AddLowCrossings(vg: VisibilityGraph, crossingVertex: VisibilityVertex, crossings: GroupBoundaryCrossing[]) {
        if ((null != crossings)) {
            let interiorVertex: VisibilityVertex = ScanSegment.GetCrossingInteriorVertex(vg, crossingVertex, crossings[0]);
