...
export class PortManager {
  ...
  private obstaclePortMap: Map<Port, ObstaclePort> = new Map<
    Port,
    ObstaclePort
  >()
  ...
  private CreateObstaclePort(obstacle: Obstacle, port: Port): ObstaclePort {
    ...
    if (
      ...
      Curve.PointRelativeToCurveLocation(
        roundedLocation,
        obstacle.InputShape.BoundaryCurve,
      )
    ) {
    ...
    if (
      ...
      PointLocation.Outside ==
        Curve.PointRelativeToCurveLocation(roundedLocation, port.Curve)
    ) {
  ...
  ConnectOobWaypointToEndpointVisibilityAtGraphBoundary(
    ...
    let dirFromGraph =
      oobWaypoint.OutOfBoundsDirectionFromGraph &
      (Direction.North | Direction.South)
    this.ConnectToGraphAtPointsCollinearWithVertices(
      oobWaypoint,
      dirFromGraph,
      endpointVertices,
    )
  ...
  private ConnectToGraphAtPointsCollinearWithVertices(
    ...
    for (const vertex of endpointVertices) {
      const graphBorderLocation = this.InBoundsGraphBoxIntersect(
        vertex.point,
        dirFromGraph,
      )
      const graphBorderVertex = this.VisGraph.FindVertex(graphBorderLocation)
      if (graphBorderVertex != null) {
        this.TransUtil.ConnectVertexToTargetVertex(
          oobWaypoint.Vertex,
          graphBorderVertex,
          dirToGraph,
  ...
  FindAncestorsAndObstaclePort(
    port: Port,
    t: {oport: ObstaclePort},
  ): Set<Shape> {
  ...
  private ActivateAncestors(
    ssAncsToUse: Set<Shape>,
    ttAncsToUse: Set<Shape>,
    shapeToObstacleMap: Map<Shape, Obstacle>,
  ) {
  ...
  public GetPortVisibilityIntersection(edgeGeometry: EdgeGeometry): Point[] {
    ...
    if (
      sourceOport.Obstacle.IsInConvexHull ||
    ...
    if (
      !sourceOport.VisibilityRectangle.intersects(
        targetOport.VisibilityRectangle,
      )
    ) {
      return null
    }
  ...
  private static GetPathPointsFromIntersectingVisibility(
    ...
    if (
      sourceEntrance.HasGroupCrossingBeforePoint(intersect) ||
      targetEntrance.HasGroupCrossingBeforePoint(intersect)
    ) {
      return null
    }
  ...
  private CreateObstaclePortEntrancesFromPoints(oport: ObstaclePort) {
    ...
    if (
      !PointComparer.Equal(location.y, curveBox.top) &&
      !PointComparer.Equal(location.y, curveBox.bottom)
    ) {
      found = true
      const hSeg = new LineSegment(
        graphBox.left,
        location.y,
        graphBox.right,
        location.y,
      ...
      this.CreatePortEntrancesAtBorderIntersections(
        curveBox,
        oport,
        location,
        wBorderIntersect,
    ...
    if (
      !PointComparer.Equal(location.x, curveBox.left) &&
      !PointComparer.Equal(location.x, curveBox.right)
    ) {
      found = true
      const vSeg = new LineSegment(
        location.x,
        graphBox.bottom,
        location.x,
        graphBox.top,
  ...
  private CreatePortEntrancesAtBorderIntersections(
    ...
    const dir: Direction = PointComparer.GetDirections(
      unpaddedBorderIntersect0,
      unpaddedBorderIntersect1,
    )
    ...
    if (!PointComparer.EqualPP(unpaddedBorderIntersect1, location)) {
      this.CreatePortEntrance(
        curveBox,
        oport,
        unpaddedBorderIntersect0,
        CompassVector.OppositeDir(dir),
      )
    }
  }
  ...
  private CreatePortEntrance(
    ...
    if (axisDistanceBetweenIntersections > GeomConstants.intersectionEpsilon) {
      ...
      const perpDirs: Direction = CompassVector.VectorDirection(
        PortManager.GetDerivative(oport, unpaddedBorderIntersect),
      )
  ...
  private CreateEntrancesForCornerPort(
    ...
    oport.CreatePortEntrance(
      location,
      CompassVector.RotateRight(outDir),
      this.ObstacleTree,
    )
  }
  ...
  private AddObstaclePortEntranceToGraph(entrance: ObstaclePortEntrance) {
    ...
    const weight: number = entrance.IsOverlapped
  ...
  private InBoundsGraphBoxIntersect(point: Point, dir: Direction): Point {
    return StaticGraphUtility.RectangleBorderIntersect(
      this.graphGenerator.ObstacleTree.GraphBox,
      point,
      dir,
    )
  }

  FindorCreateNearestPerpEdgePPDN(
    first: Point,
    second: Point,
    dir: Direction,
    weight: number,
  ...
  private FindorCreateNearestPerpEdgePPDNT(
    ...
    const edgeIntersect: Point = StaticGraphUtility.SegmentIntersectionSP(
      nearestPerpSeg,
      low,
    )
  ...
  private FindOrCreateNearestPerpEdgeFromNearestPerpSegment(
    ...
    if (tt.segsegVertex == null) {
      const edge = this.FindOrCreateSegmentIntersectionVertexAndAssociatedEdge(
        pointLocation,
        edgeIntersect,
        scanSeg,
        weight,
      ...
      return this.TransUtil.FindNextEdge(
        t.targetVertex,
        CompassVector.OppositeDir(scanSeg.ScanDirection.Dir),
      )
    }
    ...
    const dirTowardLocation: Direction = PointComparer.GetDirections(
      edgeIntersect,
      pointLocation,
    )
    let perpDir: Direction = PointComparer.GetDirections(
      tt.segsegVertex.point,
      pointLocation,
    )
    if (dirTowardLocation == perpDir) {
      ...
      return (
        this.TransUtil.FindNextEdge(
          ot.bracketSource,
          CompassVector.RotateLeft(dirTowardLocation),
        ) ??
        this.TransUtil.FindNextEdge(
          ot.bracketSource,
          CompassVector.RotateRight(dirTowardLocation),
        )
      )
    }
    ...
    if (perpendicularEdge == null) {
      ...
      return this.TransUtil.FindOrAddEdge(
        t.targetVertex,
    ...
    tt.segsegVertex = StaticGraphUtility.GetEdgeEnd(
      perpendicularEdge,
      CompassVector.OppositeDir(perpDir),
    )
    edgeIntersect = StaticGraphUtility.SegmentIntersectionPPP(
      pointLocation,
      edgeIntersect,
      tt.segsegVertex.point,
    )
  ...
  private FindOrCreateSegmentIntersectionVertexAndAssociatedEdge(
    ...
    if (intSegBefore == null) {
      ...
      return this.TransUtil.FindOrAddEdge(
        t.targetVertex,
        scanSeg.LowestVisibilityVertex,
        scanSeg.Weight,
      )
    }
    ...
    const segsegIntersect: Point = StaticGraphUtility.SegmentsIntersection(
      scanSeg,
      intSegBefore,
    )
    ...
    if (t.segsegVertex == null) {
      ...
      const newEdge = this.AddEdgeToClosestSegmentEnd(
        scanSeg,
        t.segsegVertex,
        scanSeg.Weight,
      )
      this.AddEdgeToClosestSegmentEnd(
        intSegBefore,
        t.segsegVertex,
        intSegBefore.Weight,
      )
  ...
  private AddEdgeToClosestSegmentEnd(
    ...
    if (
      PointComparer.IsPureLower(
        scanSeg.HighestVisibilityVertex.point,
        segsegVertex.point,
      )
    ) {
      return this.TransUtil.FindOrAddEdge(
    ...
    if (
      PointComparer.IsPureLower(
        segsegVertex.point,
        scanSeg.LowestVisibilityVertex.point,
      )
    ) {
      return this.TransUtil.FindOrAddEdge(
        segsegVertex,
    ...
    return this.TransUtil.FindOrAddEdgeVV(
      scanSeg.LowestVisibilityVertex,
      segsegVertex,
    )
  }
  ...
  private AddFreePointToGraph(location: Point): FreePoint {
    ...
    freePoint.IsOverlapped = this.ObstacleTree.PointIsInsideAnObstacle(
      freePoint.Point,
      this.HScanSegments.ScanDirection,
    )
    let scanSegment
    ...
    if (scanSegment != null) {
      ...
      const t: {targetVertex: VisibilityVertex} = {targetVertex: null}
      edge = this.FindOrCreateNearestPerpEdgeFromNearestPerpSegment(
        location,
        scanSegment,
        location,
        freePoint.InitialWeight,
        t,
      )
    }
    ...
    if (edge != null) {
      ...
      this.ConnectFreePointToLateralEdge(
        freePoint,
        CompassVector.RotateLeft(edgeDir),
      )
      this.ConnectFreePointToLateralEdge(
        freePoint,
        CompassVector.RotateRight(edgeDir),
      )
    } else {
  ...
  private CreateOutOfBoundsFreePoint(freePoint: FreePoint) {
    ...
    if (inboundsVertex != null) {
      ...
      const edge = this.FindorCreateNearestPerpEdgePPDN(
        oobLocation,
        inboundsLocation,
        dirFromGraph,
        ScanSegment.NormalWeight,
      ...
      if (edge != null) {
        inboundsVertex = freePoint.AddEdgeToAdjacentEdge(
          this.TransUtil,
          edge,
          dirToGraph,
          this.portSpliceLimitRectangle,
    ...
    const inboundsLeftVertex = StaticGraphUtility.FindAdjacentVertex(
      inboundsVertex,
      CompassVector.RotateLeft(dirToGraph),
    )
    ...
    const inboundsRightVertex = StaticGraphUtility.FindAdjacentVertex(
      inboundsVertex,
      CompassVector.RotateRight(dirToGraph),
    )
  ...
  private ConnectFreePointToLateralEdge(
    ...
    if (lateralEdge != null) {
      freePoint.AddEdgeToAdjacentEdge(
        this.TransUtil,
        lateralEdge,
        lateralDir,
        this.portSpliceLimitRectangle,
