...
export class ObstacleTree {
  ...
  Init(
    ...
    ancestorSets: Map<Shape, Set<Shape>>,
    idToObstacleMap: Map<Shape, Obstacle>,
  ) {
  ...
  private CreateRoot() {
    ...
    this.Root = ObstacleTree.CalculateHierarchy(
  ...
  static CalculateHierarchy(
    obstacles: Iterable<Obstacle>,
  ): RectangleNode<Obstacle, Point> {
    const rectNodes = Array.from(obstacles).map((obs) =>
      mkRectangleNode(obs, obs.VisibilityBoundingBox),
    )
  ...
  private AccumulateObstaclesForClumps() {
    ...
    const rectangularObstacles = ObstacleTree.CalculateHierarchy(
      this.GetAllObstacles().filter((obs) => !obs.IsGroup && obs.IsRectangle),
    )
  ...
  private AccumulateObstaclesForConvexHulls() {
    ...
    const allPrimaryNonGroupObstacles = ObstacleTree.CalculateHierarchy(
      this.GetAllObstacles().filter(
        (obs) => obs.IsPrimaryObstacle && !obs.IsGroup,
      ),
    )
  ...
  private AccumulateObstaclesForGroupOverlaps() {
    const groupObstacles = ObstacleTree.CalculateHierarchy(
      this.GetAllObstacles().filter((obs) => obs.IsGroup),
    )
    const allPrimaryObstacles = ObstacleTree.CalculateHierarchy(
      this.GetAllObstacles().filter((obs) => obs.IsPrimaryObstacle),
    )
  ...
  private EvaluateOverlappedPairForGroup(a: Obstacle, b: Obstacle) {
    ...
    if (a.IsRectangle && b.IsRectangle) {
      ...
      if (!b.IsGroup) {
        if (
          t.aIsInsideB ||
          ObstacleTree.FirstRectangleContainsACornerOfTheOther(
            b.VisibilityBoundingBox,
            a.VisibilityBoundingBox,
          )
        ) {
  ...
  private static FirstRectangleContainsACornerOfTheOther(
    ...
    return (
      a.contains(b.leftBottom) ||
      a.contains(b.leftTop) ||
      a.contains(b.rightTop) ||
      a.contains(b.rightBottom)
    )
  }
  ...
  private static FirstPolylineStartIsInsideSecondPolyline(
    ...
    return (
      Curve.PointRelativeToCurveLocation(first.start, second) !=
  ...
  private AddClumpToConvexHull(obstacle: Obstacle) {
    if (obstacle.isOverlapped) {
      for (const sibling of obstacle.clump.filter(
        (sib) => sib.Ordinal != obstacle.Ordinal,
      )) {
  ...
  private AddConvexHullToConvexHull(obstacle: Obstacle) {
    if (obstacle.IsInConvexHull) {
      for (const sibling of obstacle.ConvexHull.Obstacles.filter(
        (sib) => sib.Ordinal != obstacle.Ordinal,
      )) {
        this.overlapPairs.add(new IntPair(obstacle.Ordinal, sibling.Ordinal))
      }
  ...
  private static ResolveGroupAndGroupOverlap(
    ...
    if (
      a.VisibilityPolyline.boundingBox.area >
      b.VisibilityPolyline.boundingBox.area
    ) {
      ObstacleTree.ResolveGroupAndObstacleOverlap(a, b)
    } else {
  ...
  static ObstaclesIntersect(
    a: Obstacle,
    b: Obstacle,
    t: {aIsInsideB: boolean; bIsInsideA: boolean},
  ): boolean {
    ...
    t.aIsInsideB = ObstacleTree.FirstPolylineStartIsInsideSecondPolyline(
      a.VisibilityPolyline,
      b.VisibilityPolyline,
    )
    t.bIsInsideA =
      !t.aIsInsideB &&
      ObstacleTree.FirstPolylineStartIsInsideSecondPolyline(
        b.VisibilityPolyline,
        a.VisibilityPolyline,
      )
    if (a.IsRectangle && b.IsRectangle) {
    ...
    if (
      ObstacleTree.ObstaclesAreCloseEnoughToBeConsideredTouching(
        a,
        b,
        t.aIsInsideB,
        t.bIsInsideA,
  ...
  AdjustSpatialAncestors(): boolean {
    ...
    for (const group of this.GetAllGroups()) {
      ...
      for (const obstacle of this.Root.GetNodeItemsIntersectingRectangle(
        ...
        if (
          obstacle != group &&
          Curve.ClosedCurveInteriorsIntersect(
            obstacle.VisibilityPolyline,
            group.VisibilityPolyline,
          )
        ) {
          if (obstacle.IsInConvexHull) {
            Assert.assert(
  ...
  SegmentCrossesAnObstacle(startPoint: Point, endPoint: Point): boolean {
    ...
    const obstacleIntersectSeg: LineSegment = this.RestrictSegmentPrivate(
      startPoint,
      endPoint,
    )
  ...
  private RecurseRestrictRayWithObstacles(
    rectNode: RectangleNode<Obstacle, Point>,
  ) {
