...
import { VisibilityGraphGenerator } from "./VisibilityGraphGenerator";
    ...
    export class RectilinearEdgeRouter extends Algorithm {
        ...
        public AddEdgeGeometryToRoute(edgeGeometry: EdgeGeometry) {
            ...
            if (!Point.closeDistEps(GeomConstants.RoundPoint(edgeGeometry.sourcePort.Location), GeomConstants.RoundPoint(edgeGeometry.targetPort.Location))) {
        ...
        public get Obstacles(): IEnumerable<Shape> {
            return from(this.ShapeToObstacleMap.values()).select((obs) => obs.InputShape);
        }
        ...
        //  The collection of padded obstacle boundary polylines around the input shapes to route around.
         ...
         get PaddedObstacles(): IEnumerable<Polyline> {
            return from(this.ShapeToObstacleMap.values()).select((obs) => obs.PaddedPolyline);
        }
        ...
        private CreatePaddedObstacle(shape: Shape) {
            let obstacle = new Obstacle(shape, this.UseObstacleRectangles, this.Padding);
        ...
        RemoveObstacleWithoutRebuild(shape: Shape) {
            ...
            let obstacle: Obstacle = this.ShapeToObstacleMap.get(shape)
        ...
        RebuildTreeAndGraph() {
            let hadTree: boolean = (this.ObstacleTree.Root != null);
            let hadVg: boolean = (this.GraphGenerator.VisibilityGraph != null);
        ...
        private AddControlPointsAndGeneratePath(shortestPathRouter: MsmtRectilinearPath, edgePath: Path) {
            let intersectPoints: Point[] = this.PortManager.GetPortVisibilityIntersection(edgePath.EdgeGeometry);
        ...
        private static GetSingleStagePath(edgePath: Path, shortestPathRouter: MsmtRectilinearPath, sourceVertices: Array<VisibilityVertex>, targetVertices: Array<VisibilityVertex>, lastChance: boolean): boolean {
        ...
        private static EnsureNonNullPath(edgePath: Path) {
         ...
         NudgePaths(edgePaths: IEnumerable<Path>) {
            ...
            let ancestorSets = SplineRouter.GetAncestorSetsMap(this.Obstacles);
            ...
            Nudger.NudgePaths(edgePaths, this.CornerFitRadius, this.PaddedObstacles, ancestorSets, RemoveStaircases);
        ...
        private removeStaircases: boolean = true;
        ...
        public set RemoveStaircases(value: boolean)  {
         ...
         FinaliseEdgeGeometries() {
            for (let edgeGeom: EdgeGeometry of this.EdgeGeometries.Concat(this.selfEdges)) {
                ...
                if ((poly != null)) {
                    edgeGeom.Curve = RectilinearEdgeRouter.FitArcsIntoCorners(this.CornerFitRadius, poly.ToArray());
                }
        ...
        private static GetFittedArcSegs(radius: number, polyline: Point[]): IEnumerable<Ellipse> {
            ...
            for (let i: number = 1; (i 
                ...
                let ndir: Point = (leg / legLength);
                if ((Math.abs((ndir * dir)) > 0.9)) {
                    ret = new Ellipse(0, 0, polyline[i]);
                }
                ...
                yield;
                return;
                new Ellipse(0, (Math.PI / 2), axis0, axis1, (polyline[i] 
                                - (axis1 - axis0)));
                dir = ndir;
                rad0 = nrad0;
            }
