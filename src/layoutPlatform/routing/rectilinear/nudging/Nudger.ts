...
import {Point, ICurve, Rectangle} from '../../../..'
...
export class Nudger {
  ...
  UmmapPathInteriourFromStrangerObstacles(path: Path) {
    ...
    for (
      let edge = firstUnmappedEdge;
      edge != null && edge != lastUnmappedEdge;
      edge = edge.Next
    ) {
  ...
  MapPathEndAxisEdgesToTheirObstacles(path: Path) {
    ...
    if (coupleOfObstacles) {
      this.ProcessThePathStartToMapAxisEdgesToTheirObstacles(
        path,
        coupleOfObstacles[0],
      )
      this.ProcessThePathEndToMapAxisEdgesToTheirObstacles(
        path,
        coupleOfObstacles[1],
      )
    }
  ...
  ProcessThePathEndToMapAxisEdgesToTheirObstacles(
    path: Path,
    ...
    for (
      ...
      edge != null &&
      CompassVector.DirectionsAreParallel(
        edge.Direction,
        this.NudgingDirection,
      );
      edge = edge.Prev
    ) {
      this.axisEdgesToObstaclesTheyOriginatedFrom.set(
        edge.AxisEdge,
        endPolyline,
      )
    }
  ...
  ProcessThePathStartToMapAxisEdgesToTheirObstacles(
    path: Path,
  ...
  static SetWidthsOfArrowheadsForEdge(path: Path) {
    const edgeGeom = path.EdgeGeometry
  ...
  static MoveLongestSegIdealPositionsInsideFeasibleInterval(
  ...
  *GetShiftedPointsSimple(path: Path): IterableIterator<Point> {
  ...
  ShiftedPoint(point: Point, segment: LongestNudgedSegment): Point {
    ...
    return this.NudgingDirection == Direction.North
      ? new Point(t, point.y)
      : new Point(point.x, -t)
  }
  ...
  static LineSegOfLongestSeg(ls: LongestNudgedSegment, dir: Direction): ICurve {
    const projectionToDir =
      dir == Direction.East ? (p: Point) => p.x : (p: Point) => p.y
  ...
  static UpdateMinMaxWithPoint(
    mm: {min: number; max: number},
    projectionToDir: PointProjection,
    point: Point,
  ) {
  ...
  CreateConstraintsBetweenLongestSegmentsForSegment(
    segment: LongestNudgedSegment,
  ) {
  ...
  CreateConstraintsOfThePathOrder(pathOrder: IEnumerable<PathEdge>) {
    ...
    for (const pathEdge of pathOrder.where(
      ...
      if (prevEdge != null) {
        this.ConstraintTwoLongestSegs(
          prevEdge.LongestNudgedSegment,
          pathEdge.LongestNudgedSegment,
        )
      }
  ...
  ConstraintTwoLongestSegs(
    prevSeg: LongestNudgedSegment,
    seg: LongestNudgedSegment,
  ) {
  ...
  AncestorsForPort(port: Port): Set<Shape> {
    ...
    return new Set<Shape>(
      this.HierarchyOfGroups.AllHitItems(
  ...
  BoundAxisEdgesAdjacentToSourceAndTargetOnEdge(path: Path) {
    this.BoundAxisEdgeAdjacentToObstaclePort(
      path.EdgeGeometry.sourcePort,
      path.FirstEdge.AxisEdge,
    )
    this.BoundAxisEdgeAdjacentToObstaclePort(
      path.EdgeGeometry.targetPort,
      path.LastEdge.AxisEdge,
    )
  }
  ...
  CreateLongestNudgedSegments() {
    const projectionToPerp =
      this.NudgingDirection == Direction.East
        ? (p: Point) => -p.y
        : (p: Point) => p.x
  ...
  CreateLongestNudgedSegmentsForPath(
    path: Path,
  ...
  static CalculateIdealPositionsForLongestSegs(
    path: Path,
    projectionToPerp: PointProjection,
  ) {
    ...
    for (const edge of path.PathEdges()) {
      if (edge.LongestNudgedSegment != null) {
        ...
        if (ret != null) {
          let t: number
          Nudger.SetIdealPositionForSeg(
            ret,
            (t = projectionToPerp(ret.start)),
            prevOffset,
    ...
    if (ret != null) {
      Nudger.SetIdealPositionForSeg(
        ret,
        projectionToPerp(ret.Start),
        prevOffset,
        projectionToPerp(path.End),
      )
    } else if (currentLongSeg != null) {
  ...
  static ExtendPolylineToPorts(t: {points: Point[]}, path: Path) {
    Nudger.ExtendPolylineToSourcePort(t, path.EdgeGeometry.sourcePort.Location)
    Nudger.ExtendPolylineToTargetPort(t, path.EdgeGeometry.targetPort.Location)
    ...
    if (t.points.length < 2) {
      ...
      t.points[0] = path.EdgeGeometry.sourcePort.Location
      t.points[1] = path.EdgeGeometry.targetPort.Location
    }
  }
  ...
  static NudgePaths(
    ...
    for (const path of paths) {
      path.EdgeGeometry.curve = Polyline.mkFromPoints(
        Nudger.BuildPolylineForPath(path),
      )
    }
  }
