...
export class RectilinearScanLine {
    ...
    Insert(side: BasicObstacleSide, scanPos: Point): RBNode<BasicObstacleSide> {
        ...
        let node = this.SideTree.insert(side);
        // DevTraceDump(2);
        return node;
    }
    ...
    Remove(side: BasicObstacleSide, scanPos: Point) {
        ...
        Assert((null != this.Find(side)), "side does not exist in the ScanLine");
        this.linePositionAtLastInsertOrRemove = scanPos;
    ...
    NextLow(side: BasicObstacleSide): RBNode<BasicObstacleSide> {
        return this.NextLow(this.Find(side));
    }
    
    NextLow(sideNode: RBNode<BasicObstacleSide>): RBNode<BasicObstacleSide> {
        let pred = this.SideTree.previous(sideNode);
        return pred;
    }
    
    NextHigh(side: BasicObstacleSide): RBNode<BasicObstacleSide> {
        return this.NextHigh(this.Find(side));
    }
    
    NextHigh(sideNode: RBNode<BasicObstacleSide>): RBNode<BasicObstacleSide> {
        let succ = this.SideTree.next(sideNode);
        return succ;
    }
    ...
    public Compare(first: BasicObstacleSide, second: BasicObstacleSide): number {
        ...
        if ((first.Obstacle == second.Obstacle)) {
            ...
            return -1;
            // TODO: Warning!!!, inline IF is not supported ?
            (first instanceof  LowObstacleSide);
            1;
        }
        ...
        let firstIntersect: Point = VisibilityGraphGenerator.ScanLineIntersectSide(this.linePositionAtLastInsertOrRemove, first, this.scanDirection);
        let secondIntersect: Point = VisibilityGraphGenerator.ScanLineIntersectSide(this.linePositionAtLastInsertOrRemove, second, this.scanDirection);
        ...
        if ((0 == cmp)) {
            let firstIsLow: boolean = (first instanceof  LowObstacleSide);
            let secondIsLow: boolean = (second instanceof  LowObstacleSide);
    ...
    static Debug_VerifySidesDoNotIntersect(side1: BasicObstacleSide, side2: BasicObstacleSide) {
        let intersect: Point;
        ...
        let isInterior = ((((side1.Start - intersect) 
                    * (intersect - side1.End)) 
                    >= (ApproximateComparer.DistanceEpsilon * -1)) 
    ...
    toString(): string {
        ...
        Test_GetScanLineDebugCurves(): List<DebugCurve> {
