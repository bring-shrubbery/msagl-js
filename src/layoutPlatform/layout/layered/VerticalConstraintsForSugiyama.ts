...
export class VerticalConstraintsForSugiyama {
      ...
      for (let i = 0; i < this.intGraph.nodeCount; i++) {
        ...
        if (j != this.maxRepresentative)
          this.gluedUpDownIntConstraints.add(
            new IntPair(this.maxRepresentative, j),
          )
      }
    ...
    if (this.minRepresentative != -1)
      for (let i = 0; i < this.intGraph.nodeCount; i++) {
        ...
        if (j != this.minRepresentative)
          this.gluedUpDownIntConstraints.add(
            new IntPair(j, this.minRepresentative),
          )
      }
  }
  ...
  createSameLayerConstraints() {
    this.sameLayerInts = this.createIntConstraintsFromStringCouples(
      this.sameLayerConstraints,
    )
  }

  createUpDownConstraints() {
    this.upDownInts = this.createIntConstraintsFromStringCouples(
      this.upDownConstraints,
    )
  }

  createIntConstraintsFromStringCouples(
    list: [GeomNode, GeomNode][],
  ): [number, number][] {
    return list
      .map((couple) => [this.nodeIndex(couple[0]), this.nodeIndex(couple[1])])
      .filter((t) => t[0] != -1 && t[1] != -1) as [number, number][]
  }

  createMinIntConstraints() {
    this.minLayerInt = this.createIntConstraintsFromExtremeLayer(
      this.minLayerOfGeomGraph,
    )
  ...
  createMaxIntConstraints() {
    this.maxLayerInt = this.createIntConstraintsFromExtremeLayer(
  ...
  getFeedbackSet(): IEdge[] {
    ...
    return Array.from(
      this.unglueIntPairs(
        CycleRemoval.getFeedbackSetWithConstraints(
          this.gluedIntGraph,
          this.gluedUpDownIntConstraints,
        ),
      ),
    )
  }
  ...
  *unglueIntPairs(gluedEdges: IEdge[]): IterableIterator<IEdge> {
    for (const gluedEdge of gluedEdges)
      for (const ungluedEdge of this.unglueEdge(gluedEdge)) yield ungluedEdge
  }
