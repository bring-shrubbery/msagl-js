...
export class GTreeOverlapRemoval {
  ...
  OneIteration(
    ...
    if (numCrossings == 0 || scanlinePhase) {
      const additionalCrossings: number = this.FindProximityEdgesWithSweepLine(
        proximityEdges,
        nodeSizes,
        nodePositions,
      )
  ...
  FindProximityEdgesWithSweepLine(
    proximityEdges: Array<MstEdge>,
    nodeSizes: Size[],
    nodePositions: Point[],
  ): number {
    const mstLineSweeper: MstLineSweeper = new MstLineSweeper(
      proximityEdges,
      nodeSizes,
      nodePositions,
      this._overlapForLayers,
    )
...
*/
  ...
  static MoveNodePositions(
    treeEdges: Array<MstEdge>,
    nodePositions: Point[],
    rootNodeId: number,
  ) {
