...
export class MdsGraphLayout extends Algorithm {
  ...
  static ScaleToAverageEdgeLength(
    g: GeomGraph,
    x: number[],
    y: number[],
    length: (e: GeomEdge) => number,
  ...
  static LayoutGraphWithMds(
    ...
    const k: number = Math.min(
      settings.PivotNumber,
      geometryGraph.shallowNodeCount,
    )
    const iter: number = settings.GetNumberOfIterationsWithMajorization(
      geometryGraph.shallowNodeCount,
    )
    ...
    MdsGraphLayout.ScaleToAverageEdgeLength(
      geometryGraph,
      arrays.x,
      arrays.y,
      length,
    ...
    if (iter > 0) {
      ...
      MultidimensionalScaling.DistanceScalingSubset(
        d,
        arrays.x,
        arrays.y,
        w,
  ...
  //     return RectangleNode.CreateRectangleNodeOnEnumeration(
