...
export class Solver {
  ...
  private loadedVariablesAndConstraintLists: Map<
    Variable,
    ConstraintListForVariable
  > = new Map<Variable, ConstraintListForVariable>()
  ...
  private updatedConstraints: Array<[Constraint, number]> = new Array<
    [Constraint, number]
  >()
  ...
  public AddVariableANNN(
    ...
    const varNew = new Variable(
      this.nextVariableOrdinal++,
      userData,
      desiredPos,
      weight,
      scale,
    ...
    this.loadedVariablesAndConstraintLists.set(
      varNew,
      new ConstraintListForVariable(new Array<Constraint>(), 0),
    )
    return varNew
  }
  ...
  *Constraints(): IterableIterator<Constraint> {
  ...
  public AddConstraintVVNB(
    ...
    this.loadedVariablesAndConstraintLists.set(
      left,
      new ConstraintListForVariable(
        constraintsForLeftVar.Constraints,
        constraintsForLeftVar.NumberOfLeftConstraints + 1,
      ),
    )
  ...
  public SolvePar(solverParameters: Parameters): Solution {
    ...
    if (this.solverParams.OuterProjectIterationsLimit < 0) {
      ...
      this.solverParams.OuterProjectIterationsLimit =
        100 * (<number>Math.log2(this.numberOfVariables) + 1)
    }
    ...
    if (this.solverParams.InnerProjectIterationsLimit < 0) {
      ...
      this.solverParams.InnerProjectIterationsLimit =
        ...
        100 * (<number>Math.log2(this.numberOfConstraints) + 1)
    }
  ...
  private SetupConstraints() {
    ...
    for (const variable of this.loadedVariablesAndConstraintLists.keys()) {
      ...
      for (
        let loadedConstraintIndex = 0;
        loadedConstraintIndex < numAllConstraints;
        loadedConstraintIndex++
      ) {
    ...
    if (
      this.solverParams.Advanced.UseViolationCache &&
      this.solverParams.Advanced.ViolationCacheMinBlocksDivisor > 0
    ) {
      this.violationCacheMinBlockCutoff = Math.min(
        this.allBlocks.Count /
          this.solverParams.Advanced.ViolationCacheMinBlocksDivisor,
    ...
    for (let ii = 0; ii < numBlocks; ii++) {
      ...
      for (let jj = 0; jj < numVars; jj++) {
        ...
        this.solverSolution.GoalFunctionValue =
          this.solverSolution.GoalFunctionValue +
          variable.Weight * (variable.ActualPos * variable.ActualPos)
        //  +bx
        this.solverSolution.GoalFunctionValue =
          this.solverSolution.GoalFunctionValue -
          2 * (variable.Weight * (variable.DesiredPos * variable.ActualPos))
      }
    }
  }
  ...
  private Project(): boolean {
    ...
    while (maxViolatedConstraint) {
      ...
      if (
        maxViolatedConstraint.Left.Block == maxViolatedConstraint.Right.Block
      ) {
  ...
  private MergeBlocks(violatedConstraint: Constraint): Block {
    ...
    let distance: number =
      violatedConstraint.Left.OffsetInBlock +
      (violatedConstraint.Gap - violatedConstraint.Right.OffsetInBlock)
    ...
    for (
      ...
      for (const constraint of variable.LeftConstraints) {
        if (!constraint.IsActive && !constraint.IsUnsatisfiable) {
          ...
          if (greaterDistEps(violation, maxViolation)) {
            ...
            if (
              null != maxViolatedConstraint &&
              maxViolation > this.violationCache.LowViolation
            ) {
      ...
      for (const constraint of variable.RightConstraints) {
        if (
          ...
          constraint.Left.Block != this.lastModifiedBlock
        ) {
          const violation: number =
            constraint.Left.ActualPos * constraint.Left.Scale +
            (constraint.Gap -
    ...
    for (const constraint of this.allConstraints.Vector) {
      ...
      if (useViolationCache) {
        ...
        if (
          ...
          (!this.violationCache.IsFull ||
            violation > this.violationCache.LowViolation)
        ) {
        ...
        if (
          null != cacheInsertConstraint &&
          cacheInsertViolation > this.violationCache.LowViolation
        ) {
          this.violationCache.Insert(
            cacheInsertConstraint,
            cacheInsertViolation,
          )
        }
      }
