...
namespace Microsoft.Msagl.Core.Geometry {
    ...
    public class RectangleNode<TData> {
        ...
        RectangleNode(number count) {
            Count=count;
        }

        // This field provides direct  access to the value type Rectangle, which RTree and other callers
...
// ReSharper restore InconsistentNaming
        ...
        // Returns all leaves whose rectangles intersect hitRectangle (or all leaves before hitTest returns false).
          VisitTree(Func<TData, HitTestBehavior> hitTest, Rectangle hitRectangle) {
        ...
        public IEnumerable<TData> GetAllLeaves() {
         ...
         IEnumerable<RectangleNode<TData>> GetAllLeafNodes() {
        ...
        static number ChooseSeeds(IList<RectangleNode<TData>> nodes, ref Rectangle b0, ref number seed0) {
            number area = new Rectangle(b0, nodes[seed0].Rectangle).Area;
            for (number i = 2; i < nodes.Count; i++) {
                number area0 = new Rectangle(b0, nodes[i].Rectangle).Area;
                if (area0 > area) {
                    seed0 = i;
                    area = area0;
                }
            ...
            for (number i = 0; i < nodes.Count; i++) {
                if (i == seed0)
                    continue;
                number area1 = new Rectangle(nodes[seed0].Rectangle, nodes[i].Rectangle).Area;
                if (area1 > area) {
                    seed1 = i;
                    area = area1;
                }
        ...
        static  DivideNodes(IList<RectangleNode<TData>> nodes, number seed0, number seed1, List<RectangleNode<TData>> gr0, List<RectangleNode<TData>> gr1, 
            ref Rectangle box0, ref Rectangle box1, number groupSplitThreshold) {
            for (number i = 0; i < nodes.Count; i++) {
...
// ReSharper disable InconsistentNaming
                ...
                number delta0 = box0_.Area - box0.Area;

                var box1_ = new Rectangle(box1, nodes[i].Rectangle);
                number delta1 = box1_.Area - box1.Area;
// ReSharper restore InconsistentNaming
        ...
        static public  TraverseHierarchy(RectangleNode<TData> node, Action<RectangleNode<TData>> visitor) {
